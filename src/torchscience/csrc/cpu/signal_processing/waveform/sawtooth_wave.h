...
#include "../../../kernel/signal_processing/waveform/sawtooth_wave.h"

namespace torchscience::cpu::signal_processing::waveform {

inline at::Tensor sawtooth_wave(
  ...
  TORCH_CHECK(
      ...
      "sawtooth_wave: Exactly one of n or t must be provided");

  if (n.has_value()) {
    TORCH_CHECK(n.value() >= 0, "sawtooth_wave: n must be non-negative");
  }
  ...
  if (t.has_value()) {
    ...
    TORCH_CHECK(sample_rate > 0, "sawtooth_wave: sample_rate must be positive");
    auto options = at::TensorOptions()
        .dtype(dtype.value_or(at::kFloat))
        .device(device.value_or(at::kCPU));
  ...
  AT_DISPATCH_FLOATING_TYPES(output.scalar_type(), "sawtooth_wave_cpu", [&] {
    ...
    at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
      for (int64_t b = begin; b < end; ++b) {
        ...
        for (int64_t i = 0; i < n_samples; ++i) {
          out_data[b * n_samples + i] = kernel::sawtooth_wave_kernel(
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
  m.impl("sawtooth_wave", &torchscience::cpu::signal_processing::waveform::sawtooth_wave);
}
