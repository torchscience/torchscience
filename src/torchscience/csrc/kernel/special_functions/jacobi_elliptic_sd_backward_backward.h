...
#include "jacobi_elliptic_sd_backward.h"

namespace torchscience::kernel::special_functions {

// Second-order backward pass for Jacobi elliptic function sd(u, m)
//
// Uses numerical finite differences for second derivatives.
// Returns gradients w.r.t. (grad_output, u, m)

template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T> jacobi_elliptic_sd_backward_backward(
    T grad_grad_u,
    T grad_grad_m,
    T gradient,
    T u,
    ...
    T h = detail::jacobi_sd_finite_diff_step<T>();

    // Compute function values at various points
    T f_center = jacobi_elliptic_sd(u, m);
    T f_u_plus = jacobi_elliptic_sd(u + h, m);
    T f_u_minus = jacobi_elliptic_sd(u - h, m);
    T f_m_plus = jacobi_elliptic_sd(u, m + h);
    T f_m_minus = jacobi_elliptic_sd(u, m - h);

    // For mixed and second partials
    T f_u_plus_m_plus = jacobi_elliptic_sd(u + h, m + h);
    T f_u_plus_m_minus = jacobi_elliptic_sd(u + h, m - h);
    T f_u_minus_m_plus = jacobi_elliptic_sd(u - h, m + h);
    T f_u_minus_m_minus = jacobi_elliptic_sd(u - h, m - h);
...
jacobi_elliptic_sd_backward_backward(
    ...
    c10::complex<T> f_center = jacobi_elliptic_sd(u, m);
    c10::complex<T> f_u_plus = jacobi_elliptic_sd(u + h_complex, m);
    c10::complex<T> f_u_minus = jacobi_elliptic_sd(u - h_complex, m);
    c10::complex<T> f_m_plus = jacobi_elliptic_sd(u, m + h_complex);
    c10::complex<T> f_m_minus = jacobi_elliptic_sd(u, m - h_complex);

    // For mixed and second partials
    c10::complex<T> f_u_plus_m_plus = jacobi_elliptic_sd(u + h_complex, m + h_complex);
    c10::complex<T> f_u_plus_m_minus = jacobi_elliptic_sd(u + h_complex, m - h_complex);
    c10::complex<T> f_u_minus_m_plus = jacobi_elliptic_sd(u - h_complex, m + h_complex);
    c10::complex<T> f_u_minus_m_minus = jacobi_elliptic_sd(u - h_complex, m - h_complex);
