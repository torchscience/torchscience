...
template <typename T>
C10_HOST_DEVICE inline T inverse_jacobi_sn_finite_diff_step() {
    return std::pow(std::numeric_limits<T>::epsilon(), T(1.0/3.0));
}

template <>
C10_HOST_DEVICE inline float inverse_jacobi_sn_finite_diff_step<float>() {
    return 1e-3f;
}

template <>
C10_HOST_DEVICE inline double inverse_jacobi_sn_finite_diff_step<double>() {
    return 1e-6;
}
...
template <typename T>
C10_HOST_DEVICE std::tuple<T, T> inverse_jacobi_elliptic_sn_backward(
    T gradient,
    T x,
    T m
) {
...
template <typename T>
C10_HOST_DEVICE std::tuple<c10::complex<T>, c10::complex<T>> inverse_jacobi_elliptic_sn_backward(
