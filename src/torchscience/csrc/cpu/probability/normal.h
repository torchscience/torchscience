...
#include "../../kernel/probability/normal_cdf_backward_backward.h"
#include "../../kernel/probability/normal_pdf.h"
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> normal_cdf_backward_backward(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::kBFloat16, at::kHalf, x.scalar_type(), "normal_cdf_backward_backward_cpu", [&] {
        ...
        at::parallel_for(0, n, 1000, [&](int64_t begin, int64_t end) {
          for (int64_t i = begin; i < end; ++i) {
            kernel::probability::normal_cdf_backward_backward<scalar_t>(
...
at::Tensor normal_pdf(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::kBFloat16, at::kHalf, x.scalar_type(), "normal_pdf_cpu", [&] {
        auto x_data = x_b.data_ptr<scalar_t>();
        auto loc_data = loc_b.data_ptr<scalar_t>();
        auto scale_data = scale_b.data_ptr<scalar_t>();
        auto out_data = output.data_ptr<scalar_t>();
        ...
        at::parallel_for(0, n, 1000, [&](int64_t begin, int64_t end) {
          for (int64_t i = begin; i < end; ++i) {
            out_data[i] = kernel::probability::normal_pdf<scalar_t>(
                x_data[i], loc_data[i], scale_data[i]);
          }
        });
      });
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> normal_pdf_backward(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::kBFloat16, at::kHalf, x.scalar_type(), "normal_pdf_backward_cpu", [&] {
        auto grad_data = grad_b.data_ptr<scalar_t>();
        auto x_data = x_b.data_ptr<scalar_t>();
        auto loc_data = loc_b.data_ptr<scalar_t>();
        auto scale_data = scale_b.data_ptr<scalar_t>();
        ...
        at::parallel_for(0, n, 1000, [&](int64_t begin, int64_t end) {
          for (int64_t i = begin; i < end; ++i) {
            kernel::probability::normal_pdf_backward<scalar_t>(
                grad_data[i], x_data[i], loc_data[i], scale_data[i],
                grad_x_data[i], grad_loc_data[i], grad_scale_data[i]);
          }
        });
...
at::Tensor normal_ppf(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::kBFloat16, at::kHalf, p.scalar_type(), "normal_ppf_cpu", [&] {
        ...
        at::parallel_for(0, n, 1000, [&](int64_t begin, int64_t end) {
          for (int64_t i = begin; i < end; ++i) {
            out_data[i] = kernel::probability::normal_ppf<scalar_t>(
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> normal_ppf_backward(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::kBFloat16, at::kHalf, p.scalar_type(), "normal_ppf_backward_cpu", [&] {
        auto grad_data = grad_b.data_ptr<scalar_t>();
        auto p_data = p_b.data_ptr<scalar_t>();
        auto loc_data = loc_b.data_ptr<scalar_t>();
        auto scale_data = scale_b.data_ptr<scalar_t>();
        ...
        at::parallel_for(0, n, 1000, [&](int64_t begin, int64_t end) {
          for (int64_t i = begin; i < end; ++i) {
            kernel::probability::normal_ppf_backward<scalar_t>(
