...
template <typename T>
C10_HOST_DEVICE T parabolic_cylinder_u_aa_derivative(T a, T z) {
...
template <typename T>
C10_HOST_DEVICE T parabolic_cylinder_u_zz_derivative(T a, T z) {
...
template <typename T>
C10_HOST_DEVICE T parabolic_cylinder_u_az_derivative(T a, T z) {
...
// Complex versions
template <typename T>
C10_HOST_DEVICE c10::complex<T> parabolic_cylinder_u_aa_derivative(c10::complex<T> a, c10::complex<T> z) {
    const T eps = std::cbrt(pcf_eps<T>());
    T a_mag = std::abs(a);
...
template <typename T>
C10_HOST_DEVICE c10::complex<T> parabolic_cylinder_u_zz_derivative(c10::complex<T> a, c10::complex<T> z) {
    c10::complex<T> u = parabolic_cylinder_u(a, z);
    return (z * z / c10::complex<T>(T(4), T(0)) + a) * u;
}

template <typename T>
C10_HOST_DEVICE c10::complex<T> parabolic_cylinder_u_az_derivative(c10::complex<T> a, c10::complex<T> z) {
    const T eps = std::sqrt(pcf_eps<T>());
    T a_mag = std::abs(a);
    c10::complex<T> h(eps * (a_mag > T(1) ? a_mag : T(1)), T(0));
...
template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T> parabolic_cylinder_u_backward_backward(
...
template <typename T>
C10_HOST_DEVICE std::tuple<c10::complex<T>, c10::complex<T>, c10::complex<T>> parabolic_cylinder_u_backward_backward(
    c10::complex<T> gg_a,
    c10::complex<T> gg_z,
