...
template <typename T>
C10_HOST_DEVICE inline T carlson_rf_tolerance() {
    return T(1e-10);
}

template <>
C10_HOST_DEVICE inline float carlson_rf_tolerance<float>() { return 1e-5f; }

template <>
C10_HOST_DEVICE inline double carlson_rf_tolerance<double>() { return 1e-14; }

} // namespace detail

template <typename T>
C10_HOST_DEVICE T carlson_elliptic_integral_r_f(T x, T y, T z) {
...
template <typename T>
C10_HOST_DEVICE c10::complex<T> carlson_elliptic_integral_r_f(
    c10::complex<T> x,
    c10::complex<T> y,
    c10::complex<T> z
) {
