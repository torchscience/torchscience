...
def logarithmic_chirp_wave(
    ...
    *,
    ...
    """
    Logarithmic chirp (frequency sweep) waveform generator.

    Generates a chirp signal where the instantaneous frequency varies
    exponentially from f0 to f1 over the time interval [0, t1]:

        f(t) = f0 * (f1/f0)^(t/t1)

    The output signal is:

        y(t) = amplitude * cos(phi(t))

    where the phase is:

        phi(t) = 2*pi * f0 * t1 * (k^(t/t1) - 1) / ln(k) + phase
    ...
    Examples
    --------
    Generate a basic logarithmic chirp from 1 Hz to 100 Hz:

    >>> logarithmic_chirp_wave(n=1000, f0=1.0, f1=100.0, sample_rate=1000.0)

    Generate batched chirps with different starting frequencies:

    >>> f0 = torch.tensor([1.0, 5.0, 10.0])
    >>> logarithmic_chirp_wave(n=1000, f0=f0, f1=100.0, sample_rate=1000.0)  # shape (3, 1000)

    Use explicit time tensor:

    >>> t = torch.linspace(0, 1, 1000)
    >>> logarithmic_chirp_wave(t=t, f0=10.0, f1=100.0)

    Notes
    -----
    The logarithmic chirp produces a frequency sweep where equal time intervals
    correspond to equal multiplicative changes in frequency. This is useful for
    applications where perception is logarithmic (e.g., audio frequency sweeps).

    The scipy.signal.chirp function provides a similar capability with
    method='logarithmic'.

    The instantaneous phase is computed by integrating the instantaneous
    frequency f(t) = f0 * (f1/f0)^(t/t1).
    """
    ...
    result = torch.ops.torchscience.logarithmic_chirp_wave(
        n,
        t,
        f0=f0,
        f1=f1,
