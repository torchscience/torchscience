...
/**
 * Sparse CSR CUDA implementation of Hilbert transform.
 */
inline at::Tensor hilbert_transform(
    ...
    TORCH_CHECK(
        ...
        "hilbert_transform (SparseCsrCUDA) expects sparse CSR CUDA tensor"
    );
...
/**
 * Backward pass for sparse CSR CUDA Hilbert transform.
 */
inline at::Tensor hilbert_transform_backward(
    ...
    TORCH_CHECK(
        input.layout() == at::kSparseCsr && input.is_cuda(),
        "hilbert_transform_backward (SparseCsrCUDA) expects sparse CSR CUDA tensor for input"
    );
...
/**
 * Double backward pass for sparse CSR CUDA Hilbert transform.
 */
...
TORCH_LIBRARY_IMPL(torchscience, SparseCsrCUDA, module) {
    module.impl(
        "hilbert_transform",
        &torchscience::sparse::csr::cuda::integral_transform::hilbert_transform
    );
    module.impl(
        "hilbert_transform_backward",
        &torchscience::sparse::csr::cuda::integral_transform::hilbert_transform_backward
    );
    module.impl(
        "hilbert_transform_backward_backward",
        &torchscience::sparse::csr::cuda::integral_transform::hilbert_transform_backward_backward
    );
}
