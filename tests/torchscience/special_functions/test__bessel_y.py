...
class TestBesselY:
    """Tests for the Bessel function Yₙ(z) of general order n."""

    # =========================================================================
    # Forward correctness tests - integer orders
    # =========================================================================

    def test_order_zero_matches_y0(self):
        """Test Y₀(z) via bessel_y(0, z) matches bessel_y_0(z)."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        n = torch.zeros_like(z)
        result = torchscience.special_functions.bessel_y(n, z)
        expected = torchscience.special_functions.bessel_y_0(z)
        torch.testing.assert_close(result, expected, rtol=1e-10, atol=1e-10)

    def test_order_one_matches_y1(self):
        """Test Y₁(z) via bessel_y(1, z) matches bessel_y_1(z)."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        n = torch.ones_like(z)
        result = torchscience.special_functions.bessel_y(n, z)
        expected = torchscience.special_functions.bessel_y_1(z)
        torch.testing.assert_close(result, expected, rtol=1e-10, atol=1e-10)

    def test_scipy_agreement_integer_orders(self):
        """Test agreement with scipy.special.yv for integer orders."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        for order in [0, 1, 2, 3, 5, 10]:
            n = torch.full_like(z, float(order))
            result = torchscience.special_functions.bessel_y(n, z)
            expected = torch.tensor(
                [scipy.special.yv(order, x.item()) for x in z],
                dtype=torch.float64,
            )
    ...
    def test_at_zero_is_negative_infinity(self):
        """Test Yₙ(0) = -∞ for all n."""
    ...
    def test_negative_z_returns_nan_for_real(self):
        """Test that Y_n(z) for z < 0 returns NaN for real n."""
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.float64)
        z = torch.tensor([-1.0, -2.0, -3.0], dtype=torch.float64)
        result = torchscience.special_functions.bessel_y(n, z)
        # Y_n is not defined for negative real z (branch cut)
        assert torch.isnan(result).all()
    ...
    def test_negative_integer_order_symmetry(self):
        """Test Y_{-n}(z) = (-1)^n * Yₙ(z) for integer n."""
        z = torch.tensor([1.0, 2.0, 5.0], dtype=torch.float64)
        for order in [1, 2, 3, 4, 5]:
            n_pos = torch.full_like(z, float(order))
            n_neg = torch.full_like(z, float(-order))
            result_pos = torchscience.special_functions.bessel_y(n_pos, z)
            result_neg = torchscience.special_functions.bessel_y(n_neg, z)
    ...
    def test_recurrence_relation(self):
        """Test recurrence: Yₙ₋₁(z) + Yₙ₊₁(z) = (2n/z) * Yₙ(z)."""
        ...
        for order in [1.0, 2.0, 3.0, 0.5, 1.5]:
            ...
            y_n = torchscience.special_functions.bessel_y(n, z)
            y_nm1 = torchscience.special_functions.bessel_y(n_minus, z)
            y_np1 = torchscience.special_functions.bessel_y(n_plus, z)
    ...
    def test_connection_formula(self):
        """Test Yₙ(z) = [Jₙ(z)cos(nπ) - J₋ₙ(z)] / sin(nπ) for non-integer n."""
        import math

        z = torch.tensor([1.0, 2.0, 5.0], dtype=torch.float64)
        for order in [0.5, 1.5, 0.3, 2.7]:
            n = torch.full_like(z, order)
            n_neg = torch.full_like(z, -order)

            y_n = torchscience.special_functions.bessel_y(n, z)
            j_n = torchscience.special_functions.bessel_j(n, z)
            j_neg_n = torchscience.special_functions.bessel_j(n_neg, z)

            cos_npi = math.cos(order * math.pi)
            sin_npi = math.sin(order * math.pi)
            expected = (j_n * cos_npi - j_neg_n) / sin_npi
            ...
            torch.testing.assert_close(
                ...
                rtol=1e-6,
                atol=1e-10,
                msg=f"Connection formula failed for n={order}",
            )
    ...
    def test_gradcheck_z_integer_order(self):
        ...
        def func(z_):
            return torchscience.special_functions.bessel_y(n, z_)
    ...
    def test_gradcheck_both_inputs(self):
        ...
        def func(n_, z_):
            return torchscience.special_functions.bessel_y(n_, z_)

        # Note: gradient w.r.t. n is numerical; may need relaxed tolerance
        assert torch.autograd.gradcheck(
    ...
    def test_gradient_z_identity(self):
        """Verify d/dz Yₙ(z) = (Yₙ₋₁(z) - Yₙ₊₁(z)) / 2."""
        n = torch.tensor([1.0, 2.0, 0.5, 1.5], dtype=torch.float64)
        z = torch.tensor(
            [1.0, 2.0, 3.0, 4.0], dtype=torch.float64, requires_grad=True
        )

        y = torchscience.special_functions.bessel_y(n, z)
        grad = torch.autograd.grad(y.sum(), z)[0]

        n_minus = n - 1
        n_plus = n + 1
        y_nm1 = torchscience.special_functions.bessel_y(n_minus, z.detach())
        y_np1 = torchscience.special_functions.bessel_y(n_plus, z.detach())
        expected = (y_nm1 - y_np1) / 2
    ...
    def test_complex_near_real_accuracy(self):
        ...
        expected = torch.tensor(
            [
                scipy.special.yv(n[i].real.item(), z[i].item())
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real Yₙ."""
        ...
        result_real = torchscience.special_functions.bessel_y(n_real, z_real)
        result_complex = torchscience.special_functions.bessel_y(
            n_complex, z_complex
        )
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.bessel_y(n, z)
    ...
    def test_vmap(self):
        ...
        def fn(z_row):
            return torchscience.special_functions.bessel_y(n, z_row)

        result = torch.vmap(fn)(z)
        expected = torchscience.special_functions.bessel_y(n.unsqueeze(0), z)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.bessel_y)
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.float64)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.bessel_y(n, z2)
    ...
    def test_large_z_asymptotic(self):
        ...
        result = torchscience.special_functions.bessel_y(n.unsqueeze(1), z)
        expected = torch.tensor(
            [[scipy.special.yv(ni.item(), zi.item()) for zi in z] for ni in n],
            dtype=torch.float64,
        )
    ...
    def test_large_order(self):
        ...
        expected = torch.tensor(
            [
                scipy.special.yv(n[i].item(), z[i].item())
                for i in range(len(z))
            ],
            dtype=torch.float64,
        )
        torch.testing.assert_close(result, expected, rtol=1e-5, atol=1e-10)
