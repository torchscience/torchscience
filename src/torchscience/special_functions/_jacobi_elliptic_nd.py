...
def jacobi_elliptic_nd(u: Tensor, m: Tensor) -> Tensor:
    r"""
    Jacobi elliptic function nd(u, m).

    Computes the Jacobi elliptic function nd for each pair of corresponding
    elements in the input tensors.

    Mathematical Definition
    -----------------------
    The Jacobi elliptic function nd is defined as the reciprocal of dn:

    .. math::

        \text{nd}(u, m) = \frac{1}{\text{dn}(u, m)}

    where dn(u, m) is the Jacobi elliptic function delta amplitude.
    ...
    - nd(u, 0) = 1 for all u
    - nd(u, 1) = cosh(u)
    - nd(K(m), m) = 1/sqrt(1 - m), where K(m) is the complete elliptic
      integral of the first kind

    Periodicity
    -----------
    The function nd(u, m) is periodic in u with period 2K(m), where K(m)
    ...
    Algorithm
    ---------
    The implementation computes 1/dn(u, m) using the arithmetic-geometric
    mean (AGM) method for the underlying dn function.
    ...
    -------
    Tensor
        The Jacobi elliptic function nd(u, m) evaluated at the input values.
    ...
    .. warning:: Poles
       ...
       The function nd has poles where dn = 0. Near these points,
       the function values can be very large or infinite.
    ...
    See Also
    --------
    jacobi_elliptic_dn : Jacobi elliptic function dn
    ...
    """
    return torch.ops.torchscience.jacobi_elliptic_nd(u, m)
