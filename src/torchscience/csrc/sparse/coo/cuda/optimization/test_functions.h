...
namespace torchscience::sparse::coo::cuda::optimization::test_functions {

/**
 * Sparse COO CUDA implementation of the Rosenbrock function.
 *
...
inline at::Tensor rosenbrock(
    ...
    TORCH_CHECK(
        x.is_sparse(),
        "rosenbrock (SparseCUDA) expects sparse COO tensor for x"
    );
...
/**
 * Backward pass for sparse COO CUDA Rosenbrock.
 */
inline std::tuple<at::Tensor, at::Tensor, at::Tensor> rosenbrock_backward(
    ...
    TORCH_CHECK(
        x.is_sparse(),
        "rosenbrock_backward (SparseCUDA) expects sparse COO tensor for x"
    );
    ...
    at::Tensor grad_a_out = a.is_sparse() ? grad_a.to_sparse() : grad_a;
    at::Tensor grad_b_out = b.is_sparse() ? grad_b.to_sparse() : grad_b;

    return {grad_x, grad_a_out, grad_b_out};
}

/**
 * Double backward pass for sparse COO CUDA Rosenbrock.
 */
...
TORCH_LIBRARY_IMPL(torchscience, SparseCUDA, module) {
    module.impl(
        "rosenbrock",
        &torchscience::sparse::coo::cuda::optimization::test_functions::rosenbrock
    );
    module.impl(
        "rosenbrock_backward",
        &torchscience::sparse::coo::cuda::optimization::test_functions::rosenbrock_backward
    );
    module.impl(
        "rosenbrock_backward_backward",
        &torchscience::sparse::coo::cuda::optimization::test_functions::rosenbrock_backward_backward
    );
}
