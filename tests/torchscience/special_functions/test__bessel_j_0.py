...
class TestBesselJ0:
    ...
    def test_special_values(self):
        """Test special values: J₀(0) = 1, J₀(inf) = 0, J₀(nan) = nan."""
        assert torchscience.special_functions.bessel_j_0(
            torch.tensor(0.0)
    ...
    def test_scipy_agreement_small(self):
        ...
        result = torchscience.special_functions.bessel_j_0(z)
        expected = torch.tensor(
            [scipy.special.j0(x.item()) for x in z], dtype=torch.float64
        )
    ...
    def test_symmetry_even_function(self):
        """J₀(-z) = J₀(z) (even function)."""
        z = torch.tensor(
            [0.1, 0.5, 1.0, 2.0, 4.0, 4.9, 5.1, 15.0, 50.0],
            dtype=torch.float64,
        )
        result_pos = torchscience.special_functions.bessel_j_0(z)
        result_neg = torchscience.special_functions.bessel_j_0(-z)
        torch.testing.assert_close(
            result_pos, result_neg, rtol=1e-12, atol=1e-12
        )

    def test_negative_real(self):
        """Test J₀ for negative real arguments (even function)."""
        z_pos = torch.tensor([0.1, 1.0, 5.0, 10.0, 50.0], dtype=torch.float64)
        z_neg = -z_pos
        result_pos = torchscience.special_functions.bessel_j_0(z_pos)
        result_neg = torchscience.special_functions.bessel_j_0(z_neg)
        torch.testing.assert_close(
            result_pos, result_neg, rtol=1e-12, atol=1e-12
        )
    ...
    def test_gradcheck(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.bessel_j_0, z
        )
    ...
    def test_gradient_identity(self):
        """Verify d/dz J₀(z) = -J₁(z) numerically."""
        z = torch.tensor(
            [0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64, requires_grad=True
        )
        y = torchscience.special_functions.bessel_j_0(z)
        grad = torch.autograd.grad(y.sum(), z)[0]
        expected = -torchscience.special_functions.bessel_j_1(z.detach())
    ...
    def test_complex_near_real_accuracy(self):
        ...
        result = torchscience.special_functions.bessel_j_0(z_near_real)
        expected = torch.tensor(
            [scipy.special.jv(0, z.item()) for z in z_near_real],
    ...
    def test_complex_farther_from_real(self):
        ...
        result = torchscience.special_functions.bessel_j_0(z_far)
        expected = torch.tensor(
            [scipy.special.jv(0, z.item()) for z in z_far],
            dtype=torch.complex128,
        )
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real J₀."""
        x_real = torch.tensor([1.0, 2.0, 3.0, 0.5, 1.5], dtype=torch.float64)
        x_complex = x_real.to(torch.complex128)
        result_real = torchscience.special_functions.bessel_j_0(x_real)
        result_complex = torchscience.special_functions.bessel_j_0(x_complex)
        torch.testing.assert_close(
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(torchscience.special_functions.bessel_j_0)(z)
        expected = torchscience.special_functions.bessel_j_0(z)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.bessel_j_0)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.bessel_j_0(z2)
    ...
    def test_broadcasting(self):
        ...
        result = torchscience.special_functions.bessel_j_0(z1 + z2)
