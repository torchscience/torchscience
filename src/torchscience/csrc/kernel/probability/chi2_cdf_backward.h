...
// Gradient of chi2_cdf with respect to x:
// F(x; df) = P(df/2, x/2)
// dF/dx = dP/dx * (1/2) = (x/2)^(df/2-1) * exp(-x/2) / Gamma(df/2) * (1/2)
template <typename T>
T chi2_cdf_grad_x(T x, T df) {
  if (x <= T(0)) return T(0);
  T a = df / T(2);
  T half_x = x / T(2);
...
// Gradient of chi2_cdf with respect to df:
// dF/ddf = dP/da * da/ddf = dP/da * (1/2)
template <typename T>
T chi2_cdf_grad_df(T x, T df) {
  if (x <= T(0)) return T(0);
  T a = df / T(2);
  T half_x = x / T(2);
...
template <typename T>
std::tuple<T, T> chi2_cdf_backward(T grad_output, T x, T df) {
  return {
    grad_output * chi2_cdf_grad_x(x, df),
    grad_output * chi2_cdf_grad_df(x, df)
  };
}
