...
class TestStruveH:
    """Tests for the Struve function H_n(z) of general order n."""
    ...
    def test_scipy_agreement_integer_orders(self):
        """Test agreement with scipy.special.struve for integer orders."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        for order in [0, 1, 2, 3, 5]:
            n = torch.full_like(z, float(order))
            result = torchscience.special_functions.struve_h(n, z)
            expected = torch.tensor(
                [scipy.special.struve(order, x.item()) for x in z],
                dtype=torch.float64,
            )
    ...
    def test_negative_z_integer_order(self):
        """Test H_n(-z) = (-1)^(n+1) * H_n(z) for integer n."""
        z = torch.tensor([1.0, 2.0, 5.0], dtype=torch.float64)
        for order in [0, 1, 2, 3, 4]:
            n = torch.full_like(z, float(order))
            result_pos = torchscience.special_functions.struve_h(n, z)
            result_neg = torchscience.special_functions.struve_h(n, -z)
    ...
    def test_gradcheck_z_integer_order(self):
        ...
        def func(z_):
            return torchscience.special_functions.struve_h(n, z_)
    ...
    def test_gradcheck_both_inputs(self):
        ...
        def func(n_, z_):
            return torchscience.special_functions.struve_h(n_, z_)
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real H_n."""
        ...
        result_real = torchscience.special_functions.struve_h(n_real, z_real)
        result_complex = torchscience.special_functions.struve_h(
            n_complex, z_complex
        )
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.struve_h(n, z)
    ...
    def test_vmap(self):
        ...
        def fn(z_row):
            return torchscience.special_functions.struve_h(n, z_row)

        result = torch.vmap(fn)(z)
        expected = torchscience.special_functions.struve_h(n.unsqueeze(0), z)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.struve_h)
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.float64)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.struve_h(n, z2)
