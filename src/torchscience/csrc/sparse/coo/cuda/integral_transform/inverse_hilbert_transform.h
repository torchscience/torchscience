...
namespace torchscience::sparse::coo::cuda::integral_transform {

/**
 * Sparse COO CUDA implementation of inverse Hilbert transform.
 */
inline at::Tensor inverse_hilbert_transform(
    ...
    TORCH_CHECK(
        input.is_sparse() && input.is_cuda(),
        "inverse_hilbert_transform (SparseCUDA) expects sparse COO CUDA tensor"
    );
...
/**
 * Backward pass for sparse COO CUDA inverse Hilbert transform.
 */
inline at::Tensor inverse_hilbert_transform_backward(
    ...
    TORCH_CHECK(
        input.is_sparse() && input.is_cuda(),
        "inverse_hilbert_transform_backward (SparseCUDA) expects sparse COO CUDA tensor for input"
    );
...
/**
 * Double backward pass for sparse COO CUDA inverse Hilbert transform.
 */
...
TORCH_LIBRARY_IMPL(torchscience, SparseCUDA, module) {
    module.impl(
        ...
        &torchscience::sparse::coo::cuda::integral_transform::inverse_hilbert_transform
    );
    module.impl(
        "inverse_hilbert_transform_backward",
        &torchscience::sparse::coo::cuda::integral_transform::inverse_hilbert_transform_backward
    );
    module.impl(
        "inverse_hilbert_transform_backward_backward",
        &torchscience::sparse::coo::cuda::integral_transform::inverse_hilbert_transform_backward_backward
    );
}
