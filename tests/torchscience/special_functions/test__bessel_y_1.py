...
class TestBesselY1:
    """Tests for the Bessel function of the second kind Y₁."""
    ...
    def test_special_values(self):
        """Test special values: Y₁(0) = -inf, Y₁(inf) = 0, Y₁(nan) = nan."""
        # Y₁(0) = -infinity (logarithmic singularity)
        assert torchscience.special_functions.bessel_y_1(
            torch.tensor(0.0)
        ).item() == float("-inf")
        # Y₁(+inf) = 0 (oscillatory decay)
        assert torchscience.special_functions.bessel_y_1(
    ...
    def test_scipy_agreement_small(self):
        ...
        z = torch.tensor(
            [0.1, 0.5, 1.0, 2.0, 3.0, 4.0, 4.9], dtype=torch.float64
        )
        result = torchscience.special_functions.bessel_y_1(z)
        expected = torch.tensor(
            [scipy.special.y1(x.item()) for x in z], dtype=torch.float64
        )
    ...
    def test_region_boundary(self):
        """Test accuracy near the |z|=5 boundary where approximations switch."""
        z = torch.tensor(
    ...
    def test_negative_real_returns_nan(self):
        """Y₁(z) for negative real z should return NaN (not on principal branch)."""
        z = torch.tensor([-0.1, -1.0, -5.0, -10.0], dtype=torch.float64)
        result = torchscience.special_functions.bessel_y_1(z)
        assert result.isnan().all()

    def test_near_zero_singularity(self):
        """Test behavior near z=0 singularity: Y₁(z) -> -2/(πz) for small z."""
    ...
    def test_zeros_of_y1(self):
        """Test near the zeros of Y₁ (first few: ~2.1971, ~5.4297, ~8.5960)."""
        # First few zeros of Y₁
        zeros = torch.tensor(
            [2.197141, 5.429681, 8.596006], dtype=torch.float64
        )
        result = torchscience.special_functions.bessel_y_1(zeros)
        # Should be near zero
        torch.testing.assert_close(
            result, torch.zeros_like(result), rtol=1e-5, atol=1e-5
        )
    ...
    def test_gradcheck(self):
        ...
        # Avoid z=0 where Y₁ has a singularity
        z = (
            torch.rand(10, dtype=torch.float64) * 4 + 0.5
        )  # values in [0.5, 4.5]
        z = z.clone().requires_grad_(True)
        assert torch.autograd.gradcheck(
            torchscience.special_functions.bessel_y_1, z
        )
    ...
    def test_gradient_identity(self):
        """Verify d/dz Y₁(z) = Y₀(z) - Y₁(z)/z numerically."""
        z = torch.tensor(
            [0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64, requires_grad=True
        )
        y = torchscience.special_functions.bessel_y_1(z)
        grad = torch.autograd.grad(y.sum(), z)[0]
        z_detach = z.detach()
        expected = (
            torchscience.special_functions.bessel_y_0(z_detach)
            - torchscience.special_functions.bessel_y_1(z_detach) / z_detach
        )
    ...
    def test_complex_dtype(self):
        ...
        # Ensure positive real part to avoid branch cut
    ...
    def test_complex_near_real_accuracy(self):
        ...
        result = torchscience.special_functions.bessel_y_1(z_near_real)
        expected = torch.tensor(
            [scipy.special.yv(1, z.item()) for z in z_near_real],
    ...
    def test_complex_farther_from_real(self):
        ...
        result = torchscience.special_functions.bessel_y_1(z_far)
        expected = torch.tensor(
            [scipy.special.yv(1, z.item()) for z in z_far],
            dtype=torch.complex128,
        )
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on positive real axis match real Y₁."""
        x_real = torch.tensor([0.5, 1.0, 2.0, 3.0, 5.0], dtype=torch.float64)
        x_complex = x_real.to(torch.complex128)
        result_real = torchscience.special_functions.bessel_y_1(x_real)
        result_complex = torchscience.special_functions.bessel_y_1(x_complex)
        torch.testing.assert_close(
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(torchscience.special_functions.bessel_y_1)(z)
        expected = torchscience.special_functions.bessel_y_1(z)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.bessel_y_1)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.bessel_y_1(z2)
    ...
    def test_broadcasting(self):
        ...
        result = torchscience.special_functions.bessel_y_1(z1 + z2)
