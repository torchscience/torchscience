import torch
from torch import Tensor


def jacobi_elliptic_cs(u: Tensor, m: Tensor) -> Tensor:
    r"""
    Jacobi elliptic function cs(u, m).

    Computes the Jacobi elliptic cs function evaluated at each element.

    Mathematical Definition
    -----------------------
    The Jacobi elliptic cs function is defined as:

    .. math::

       \mathrm{cs}(u, m) = \frac{\mathrm{cn}(u, m)}{\mathrm{sn}(u, m)}

    where:
    - :math:`\mathrm{cn}(u, m) = \cos(\mathrm{am}(u, m))`
    - :math:`\mathrm{sn}(u, m) = \sin(\mathrm{am}(u, m))`
    - :math:`\mathrm{am}(u, m)` is the Jacobi amplitude function

    Domain
    ------
    - u: real or complex
    - m: 0 <= m <= 1 for real inputs (complex plane otherwise)

    Special Values
    --------------
    - cs(u, 0) = cos(u) / sin(u) = cot(u) (since cn(u,0) = cos(u) and sn(u,0) = sin(u))
    - cs(u, 1) = sech(u) / tanh(u) = 1/sinh(u) = csch(u)

    Poles
    -----
    cs has poles where sn(u, m) = 0, i.e., at u = 2nK(m) for integer n,
    where K(m) is the complete elliptic integral of the first kind.
    This includes u = 0.

    Periodicity
    -----------
    - cs(u + 4K(m), m) = cs(u, m)

    Parity
    ------
    - cs(-u, m) = -cs(u, m) (odd function in u)

    Algorithm
    ---------
    Computed as the ratio of cn(u, m) / sn(u, m), where cn and sn are
    computed using the AGM method.

    - Provides consistent results across CPU and CUDA devices.
    - Half-precision types (float16, bfloat16) compute in float32 for accuracy.

    Applications
    ------------
    Jacobi elliptic functions appear in:
    - Nonlinear wave equations (KdV, sine-Gordon)
    - Pendulum motion (exact solutions)
    - Conformal mapping
    - Elliptic filter design
    - Soliton solutions in integrable systems

    Dtype Promotion
    ---------------
    - All inputs are promoted to a common dtype
    - Supports float16, bfloat16, float32, float64
    - Supports complex64, complex128

    Autograd Support
    ----------------
    Gradients are fully supported when inputs require grad.
    Second-order derivatives (gradgradcheck) are also supported.

    Parameters
    ----------
    u : Tensor
        The argument tensor. Broadcasting with m is supported.
    m : Tensor
        The parameter tensor. Must satisfy 0 <= m <= 1 for real inputs.
        Broadcasting with u is supported.

    Returns
    -------
    Tensor
        The Jacobi elliptic cs function evaluated at the input values.
        Output dtype follows promotion rules.

    Examples
    --------
    Basic usage:

    >>> u = torch.tensor([0.5, 1.0, 1.5])
    >>> m = torch.tensor([0.5])
    >>> jacobi_elliptic_cs(u, m)
    tensor([1.8998, 1.0681, 0.5783])

    At m = 0, cs reduces to cot:

    >>> u = torch.tensor([0.5, 1.0, 1.5])
    >>> m = torch.tensor([0.0])
    >>> jacobi_elliptic_cs(u, m)
    tensor([1.8305, 0.6421, 0.0707])
    >>> 1.0 / torch.tan(u)
    tensor([1.8305, 0.6421, 0.0707])

    At m = 1, cs reduces to csch:

    >>> u = torch.tensor([0.5, 1.0, 2.0])
    >>> m = torch.tensor([1.0])
    >>> jacobi_elliptic_cs(u, m)
    tensor([1.9190, 0.8509, 0.2757])
    >>> 1.0 / torch.sinh(u)
    tensor([1.9190, 0.8509, 0.2757])

    Complex input:

    >>> u = torch.tensor([1.0 + 0.5j])
    >>> m = torch.tensor([0.5 + 0.0j])
    >>> jacobi_elliptic_cs(u, m)
    tensor([0.9006-0.5265j])

    Autograd:

    >>> u = torch.tensor([1.0], requires_grad=True)
    >>> m = torch.tensor([0.5])
    >>> y = jacobi_elliptic_cs(u, m)
    >>> y.backward()
    >>> u.grad
    tensor([-1.3099])

    Notes
    -----
    - The function is computed as cn(u, m) / sn(u, m).
    - For m near 0 or 1, special cases are used for better numerical stability.
    - The implementation handles both real and complex arguments.
    - Has a pole at u = 0 (odd function).

    See Also
    --------
    jacobi_elliptic_cn : Jacobi elliptic cn function
    jacobi_elliptic_sn : Jacobi elliptic sn function
    jacobi_elliptic_dn : Jacobi elliptic dn function
    jacobi_elliptic_dc : Jacobi elliptic dc function
    jacobi_elliptic_ds : Jacobi elliptic ds function
    """
    return torch.ops.torchscience.jacobi_elliptic_cs(u, m)
