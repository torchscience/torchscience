...
inline at::Tensor violet_noise(
  ...
  TORCH_CHECK(size.size() > 0, "violet_noise: size must be non-empty");
  for (auto s : size) {
    TORCH_CHECK(s >= 0, "violet_noise: size elements must be non-negative, got ", s);
  }
  ...
  // Violet noise (α=-2): amplitude scales as f, so S[k] = k/N for k > 0, S[0] = 0
  ...
  // Scaling: k/N for k > 0, 0 for k = 0
  at::Tensor scaling = at::where(
    freq_indices > 0,
    freq_indices / static_cast<double>(n),
  ...
  // For violet noise, the theoretical variance is sum((k/N)²) for k=1..N/2
...
TORCH_LIBRARY_IMPL(torchscience, CompositeExplicitAutograd, module) {
  module.impl("violet_noise", &torchscience::cpu::signal_processing::noise::violet_noise);
}
