...
template <typename T>
inline T weierstrass_sigma_finite_diff_step() {
    return std::pow(std::numeric_limits<T>::epsilon(), T(1.0/3.0));
}

template <>
inline float weierstrass_sigma_finite_diff_step<float>() {
    return 1e-4f;
}

template <>
inline double weierstrass_sigma_finite_diff_step<double>() {
    return 1e-7;
}
...
template <typename T>
std::tuple<T, T, T> weierstrass_sigma_backward(
    T gradient,
    T z,
    T g2,
    T g3
...
template <typename T>
std::tuple<c10::complex<T>, c10::complex<T>, c10::complex<T>> weierstrass_sigma_backward(
