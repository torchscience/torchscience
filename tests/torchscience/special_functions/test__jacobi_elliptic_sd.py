...
class TestJacobiEllipticSd(OpTestCase):
    @property
    def descriptor(self) -> OperatorDescriptor:
        return OperatorDescriptor(
            name="jacobi_elliptic_sd",
            func=torchscience.special_functions.jacobi_elliptic_sd,
            arity=2,
            input_specs=[
            ...
            special_values=[
                SpecialValue(
                    inputs=(0.0, 0.5), expected=0.0, description="sd(0, m) = 0"
                ),
            ],
    ...
    def test_forward_against_mpmath(self):
        ...
        for u_val, m_val in test_cases:
            ...
            result = torchscience.special_functions.jacobi_elliptic_sd(u, m)
            expected = float(mpmath.ellipfun("sd", u_val, m=m_val))
    ...
    def test_quotient_identity(self):
        ...
        torch.testing.assert_close(sd, sn / dn, rtol=1e-10, atol=1e-10)
