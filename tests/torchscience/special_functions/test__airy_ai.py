...
class TestAiryAi:
    """Tests for the Airy function Ai(x)."""
    ...
    def test_special_values(self):
        """Test special values: Ai(0) ~ 0.35503, Ai(inf) = 0, Ai(nan) = nan."""
        # Ai(0) = 1/(3^(2/3) * Gamma(2/3)) ~ 0.35502805388781724
        result = torchscience.special_functions.airy_ai(
            torch.tensor(0.0, dtype=torch.float64)
        )
        assert result.item() == pytest.approx(0.35502805388781724, rel=1e-10)

        # Ai(+inf) = 0
        assert torchscience.special_functions.airy_ai(
    ...
    def test_scipy_agreement_positive(self):
        ...
        result = torchscience.special_functions.airy_ai(x)
        # scipy.special.airy returns (Ai, Ai', Bi, Bi')
        expected = torch.tensor(
            [scipy.special.airy(xi.item())[0] for xi in x], dtype=torch.float64
        )
    ...
    def test_scipy_agreement_large_positive(self):
        ...
        # Large x leads to very small values, use relative tolerance
        torch.testing.assert_close(result, expected, rtol=1e-6, atol=1e-15)
    ...
    def test_near_zeros(self):
        """Test behavior near zeros of Ai(x)."""
        # First few zeros of Ai: approximately -2.338, -4.088, -5.521
        zeros = torch.tensor(
            [-2.338107, -4.087949, -5.520560], dtype=torch.float64
        )
        result = torchscience.special_functions.airy_ai(zeros)
    ...
    def test_gradcheck(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.airy_ai, x, eps=1e-6
        )
    ...
    def test_gradient_vs_scipy(self):
        """Verify d/dx Ai(x) = Ai'(x) using scipy reference."""
        x = torch.tensor(
            [0.5, 1.0, 2.0, -0.5, -1.0, -2.0],
            dtype=torch.float64,
            requires_grad=True,
        )
        y = torchscience.special_functions.airy_ai(x)
        grad = torch.autograd.grad(y.sum(), x)[0]
        # scipy.special.airy returns (Ai, Ai', Bi, Bi')
        expected = torch.tensor(
            [scipy.special.airy(xi.item())[1] for xi in x.detach()],
            dtype=torch.float64,
        )
        torch.testing.assert_close(grad, expected, rtol=1e-6, atol=1e-8)

    def test_airy_differential_equation(self):
        """Verify the Airy differential equation: Ai''(x) = x * Ai(x)."""
        x = torch.tensor(
            [0.5, 1.0, 2.0, -1.0, -2.0],
            dtype=torch.float64,
            requires_grad=True,
    ...
    def test_complex_dtype(self):
        ...
        result = torchscience.special_functions.airy_ai(z)
    ...
    def test_complex_near_real_accuracy(self):
        ...
        result = torchscience.special_functions.airy_ai(z_near_real)
        expected = torch.tensor(
            [scipy.special.airy(zi.item())[0] for zi in z_near_real],
            dtype=torch.complex128,
        )
        torch.testing.assert_close(result, expected, rtol=1e-8, atol=1e-10)

    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real Ai."""
        x_real = torch.tensor([0.5, 1.0, 2.0, -0.5, -1.0], dtype=torch.float64)
        x_complex = x_real.to(torch.complex128)
        result_real = torchscience.special_functions.airy_ai(x_real)
        result_complex = torchscience.special_functions.airy_ai(x_complex)
        torch.testing.assert_close(
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.airy_ai(x)
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(torchscience.special_functions.airy_ai)(x)
        expected = torchscience.special_functions.airy_ai(x)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.airy_ai)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.airy_ai(x2)
    ...
    def test_broadcasting(self):
        ...
        result = torchscience.special_functions.airy_ai(x1 + x2)
