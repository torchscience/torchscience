...
class TestSphericalHankel2:
    ...
    def test_definition_general_orders(self):
        """Test h_n^(2)(z) = j_n(z) - i*y_n(z) for various orders."""
    ...
    def test_at_zero_is_nan(self):
        """Test that h_n^(2)(0) returns NaN (singularity)."""
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.complex128)
        z = torch.zeros_like(n)

        result = torchscience.special_functions.spherical_hankel_2(n, z)

        assert result.isnan().all(), "h_n^(2)(0) should be NaN"
    ...
    def test_recurrence_relation(self):
        ...
        for order in [1.0, 2.0, 3.0]:
            ...
            h_n = torchscience.special_functions.spherical_hankel_2(n, z)
            h_nm1 = torchscience.special_functions.spherical_hankel_2(
                n_minus, z
            )
            h_np1 = torchscience.special_functions.spherical_hankel_2(
                n_plus, z
            )
    ...
    def test_derivative_relation(self):
        ...
        for order in [0.0, 1.0, 2.0]:
            ...
            z_plus = z + eps
            z_minus = z - eps
            h_plus = torchscience.special_functions.spherical_hankel_2(
                n, z_plus
            )
            h_minus = torchscience.special_functions.spherical_hankel_2(
                n, z_minus
            )
    ...
    # =========================================================================
    # Relation to first kind tests
    # =========================================================================

    def test_conjugate_relation_to_first_kind(self):
        """Test h_n^(2)(x) = conj(h_n^(1)(x)) for real x."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0], dtype=torch.complex128)

        for order in [0.0, 1.0, 2.0]:
            n = torch.full_like(z, order)

            h1 = torchscience.special_functions.spherical_hankel_1(n, z)
            h2 = torchscience.special_functions.spherical_hankel_2(n, z)

            # For real z, h_n^(2) should be complex conjugate of h_n^(1)
            expected = torch.conj(h1)
            ...
            torch.testing.assert_close(
                ...
                rtol=1e-10,
                atol=1e-10,
                msg=f"Conjugate relation failed for n={order}",
            )
    ...
    def test_large_argument_decay(self):
        """Test that |h_n^(2)(z)| decays like 1/z for large real z."""
        ...
        results = []
        for z in z_values:
            h = torchscience.special_functions.spherical_hankel_2(
                n, z.unsqueeze(0)
            )
            results.append(h.abs().item())

        # For large z, |h_n^(2)(z)| ~ 1/z
