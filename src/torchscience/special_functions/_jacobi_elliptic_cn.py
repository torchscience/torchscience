...
def jacobi_elliptic_cn(u: Tensor, k: Tensor, *, out: Tensor | None = None) -> Tensor:
    """
    ...
    -------
    Tensor, shape=(...)
        The Jacobi elliptic function cn evaluated at each element pair.
    ...
    Examples
    --------
    >>> jacobi_elliptic_cn(torch.tensor([0.0]), torch.tensor([0.5]))
    tensor([1.])

    >>> jacobi_elliptic_cn(torch.tensor([1.0]), torch.tensor([0.0]))
    tensor([0.5403])
    """
    output: Tensor = torch.ops.torchscience._jacobi_elliptic_cn(u, k)
