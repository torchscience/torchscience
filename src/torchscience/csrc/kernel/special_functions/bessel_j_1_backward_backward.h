...
template <typename T>
C10_HOST_DEVICE std::tuple<T, T> bessel_j_1_backward_backward(T gg_z, T grad_output, T z) {
    T j0 = bessel_j_0(z);
    T j1 = bessel_j_1(z);
...
template <typename T>
C10_HOST_DEVICE std::tuple<c10::complex<T>, c10::complex<T>> bessel_j_1_backward_backward(
