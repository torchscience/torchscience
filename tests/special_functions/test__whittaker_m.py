...
class TestWhittakerM:
    """Tests for the Whittaker M function."""

    def test_known_values(self):
        """Test against known mathematical values."""
        # M_{0,0}(1) = exp(-1/2) * 1F1(1/2, 1, 1)
        kappa = torch.tensor([0.0])
        mu = torch.tensor([0.0])
        z = torch.tensor([1.0])
        result = whittaker_m(kappa, mu, z)
        # The result should be finite and positive
    ...
    def test_reference_scipy(self):
        ...
        expected = torch.tensor([
            scipy.special.whittaker_m(k, m, x)
    ...
    def test_gradcheck(self):
        ...
        torch.autograd.gradcheck(
            whittaker_m, (kappa, mu, z), eps=1e-6, atol=1e-4, rtol=1e-3
        )
