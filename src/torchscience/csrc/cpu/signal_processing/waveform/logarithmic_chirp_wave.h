...
#include "../../../kernel/signal_processing/waveform/logarithmic_chirp_wave.h"

namespace torchscience::cpu::signal_processing::waveform {

inline at::Tensor logarithmic_chirp_wave(
  ...
  TORCH_CHECK(
      ...
      "logarithmic_chirp_wave: Exactly one of n or t must be provided");

  if (n.has_value()) {
    TORCH_CHECK(n.value() >= 0, "logarithmic_chirp_wave: n must be non-negative, got ", n.value());
  }

  TORCH_CHECK(t1 > 0, "logarithmic_chirp_wave: t1 must be positive, got ", t1);
  ...
  if (t.has_value()) {
    ...
    TORCH_CHECK(sample_rate > 0, "logarithmic_chirp_wave: sample_rate must be positive");
    auto options = at::TensorOptions()
        .dtype(dtype.value_or(at::kFloat))
        .device(device.value_or(at::kCPU));
  ...
  auto broadcast_shape = at::infer_size(
      at::infer_size(
          at::infer_size(f0.sizes(), f1.sizes()),
          amplitude.sizes()),
      phase.sizes());

  // Expand parameters to broadcast shape
  auto f0_exp = f0.expand(broadcast_shape).contiguous();
  auto f1_exp = f1.expand(broadcast_shape).contiguous();
  ...
  AT_DISPATCH_FLOATING_TYPES(output.scalar_type(), "logarithmic_chirp_wave_cpu", [&] {
    auto time_data = time.data_ptr<scalar_t>();
    auto f0_data = f0_exp.data_ptr<scalar_t>();
    auto f1_data = f1_exp.data_ptr<scalar_t>();
    auto amp_data = amp_exp.data_ptr<scalar_t>();
    auto phase_data = phase_exp.data_ptr<scalar_t>();
    auto out_data = output.data_ptr<scalar_t>();

    scalar_t t1_val = static_cast<scalar_t>(t1);

    at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
      for (int64_t b = begin; b < end; ++b) {
        scalar_t f0_val = f0_data[b];
        scalar_t f1_val = f1_data[b];
        scalar_t amp = amp_data[b];
        scalar_t ph = phase_data[b];

        for (int64_t i = 0; i < n_samples; ++i) {
          out_data[b * n_samples + i] = kernel::logarithmic_chirp_wave_kernel(
              time_data[i], f0_val, f1_val, t1_val, amp, ph);
        }
      }
    });
  });
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
  m.impl("logarithmic_chirp_wave", &torchscience::cpu::signal_processing::waveform::logarithmic_chirp_wave);
}
