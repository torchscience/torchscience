...
class TestKelvinKer:
    """Tests for the Kelvin function ker(x)."""
    ...
    def test_special_values(self):
        """Test special values: ker(0) = inf, ker(nan) = nan."""
        assert torchscience.special_functions.kelvin_ker(
            torch.tensor(0.0)
        ).item() == float("inf")
        assert torchscience.special_functions.kelvin_ker(
    ...
    def test_scipy_agreement_small(self):
        ...
        result = torchscience.special_functions.kelvin_ker(x)
        expected = torch.tensor(
            [scipy.special.ker(val.item()) for val in x], dtype=torch.float64
        )
    ...
    def test_symmetry_even_function(self):
        """ker(-x) = ker(x) (even function)."""
        x = torch.tensor(
            [0.1, 0.5, 1.0, 2.0, 4.0, 5.0, 7.0],
            dtype=torch.float64,
        )
        result_pos = torchscience.special_functions.kelvin_ker(x)
        result_neg = torchscience.special_functions.kelvin_ker(-x)
    ...
    def test_known_values(self):
        ...
        # Reference values from scipy.special.ker
        test_cases = [
            (0.5, 0.8559058721186341),
            (1.0, 0.2867062087283160),
            (2.0, -0.0416645139915096),
            (5.0, -0.0115117271994922),
        ]
    ...
    def test_large_x_decay(self):
        """Test that ker(x) -> 0 as x -> infinity (exponential decay)."""
    ...
    def test_gradcheck(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.kelvin_ker, x, rtol=1e-4, atol=1e-4
        )
    ...
    def test_gradgradcheck(self):
        ...
        assert torch.autograd.gradgradcheck(
            torchscience.special_functions.kelvin_ker, x, rtol=1e-3, atol=1e-3
        )

    def test_gradient_sign(self):
        """Verify gradient sign at known points."""
        # ker(x) is generally decreasing for small x (negative derivative)
        x = torch.tensor([1.0], dtype=torch.float64, requires_grad=True)
        y = torchscience.special_functions.kelvin_ker(x)
        grad = torch.autograd.grad(y.sum(), x)[0]
        # ker'(1) should be negative
        assert grad.item() < 0
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real ker."""
        x_real = torch.tensor([0.5, 1.0, 2.0, 3.0], dtype=torch.float64)
        x_complex = x_real.to(torch.complex128)
        result_real = torchscience.special_functions.kelvin_ker(x_real)
        result_complex = torchscience.special_functions.kelvin_ker(x_complex)
        torch.testing.assert_close(
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.kelvin_ker(x)
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(torchscience.special_functions.kelvin_ker)(x)
        expected = torchscience.special_functions.kelvin_ker(x)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.kelvin_ker)
        x = (
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.kelvin_ker(x2)
    ...
    def test_broadcasting(self):
        ...
        result = torchscience.special_functions.kelvin_ker(x1 + x2)
