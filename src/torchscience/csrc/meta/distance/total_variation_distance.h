...
inline at::Tensor total_variation_distance(
    ...
    if (pairwise) {
        TORCH_CHECK(
            p.dim() == 2 && q.dim() == 2,
            "total_variation_distance: pairwise mode requires 2D tensors"
        );
...
TORCH_LIBRARY_IMPL(torchscience, Meta, m) {
    m.impl("total_variation_distance", &torchscience::meta::distance::total_variation_distance);
    m.impl("total_variation_distance_backward", &torchscience::meta::distance::total_variation_distance_backward);
}
