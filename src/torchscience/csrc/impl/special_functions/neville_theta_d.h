...
T neville_theta_d(T k, T u) {
  return boost::math::neville_theta_d(k, u);
}
...
std::tuple<T, T> neville_theta_d_backward(T k, T u) {
  ...
  T theta_k_plus = boost::math::neville_theta_d(k + eps, u);
  T theta_k_minus = boost::math::neville_theta_d(std::max(k - eps, T(0)), u);
  T grad_k = (theta_k_plus - theta_k_minus) / (T(2) * eps);

  T theta_u_plus = boost::math::neville_theta_d(k, u + eps);
  T theta_u_minus = boost::math::neville_theta_d(k, u - eps);
