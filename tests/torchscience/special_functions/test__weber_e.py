...
class TestWeberE:
    ...
    def test_forward_negative_z(self):
        """Test Weber function with negative z."""
    ...
    def test_gradcheck_z(self):
        ...
        def func(z_):
            return torchscience.special_functions.weber_e(n, z_)
    ...
    def test_gradcheck_n(self):
        ...
        def func(n_):
            return torchscience.special_functions.weber_e(n_, z)
    ...
    def test_derivative_formula_z(self):
        """Test that dE_nu/dz = (E_{nu-1} - E_{nu+1})/2."""
        n = torch.tensor([1.0], dtype=torch.float64)
        z = torch.tensor([2.0], dtype=torch.float64, requires_grad=True)
        result = torchscience.special_functions.weber_e(n, z)
        result.backward()

        # Expected: (E_{n-1}(z) - E_{n+1}(z)) / 2
        e_n_minus_1 = torchscience.special_functions.weber_e(
            n - 1.0, z.detach()
        )
        e_n_plus_1 = torchscience.special_functions.weber_e(
            n + 1.0, z.detach()
        )
        expected_grad = 0.5 * (e_n_minus_1 - e_n_plus_1)
    ...
    def test_gradcheck_both(self):
        ...
        def func(n_, z_):
            return torchscience.special_functions.weber_e(n_, z_)

        assert torch.autograd.gradcheck(
            func, (n, z), eps=1e-6, atol=1e-4, rtol=1e-3
        )

    def test_anger_weber_pair(self):
        """Test that Anger and Weber functions are paired correctly."""
        n = torch.tensor([0.5, 1.5], dtype=torch.float64)
        z = torch.tensor([1.0, 2.0], dtype=torch.float64)

        # Both should return finite values
        anger = torchscience.special_functions.anger_j(n, z)
        weber = torchscience.special_functions.weber_e(n, z)

        assert torch.all(torch.isfinite(anger))
        assert torch.all(torch.isfinite(weber))
