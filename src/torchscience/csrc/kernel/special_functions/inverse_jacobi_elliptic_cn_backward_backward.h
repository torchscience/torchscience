...
#include "inverse_jacobi_elliptic_cn.h"
#include "inverse_jacobi_elliptic_cn_backward.h"

namespace torchscience::kernel::special_functions {

// Second-order backward pass for inverse Jacobi elliptic function arccn(x, m)
//
// Uses numerical finite differences for second derivatives.
// Returns gradients w.r.t. (grad_output, x, m)

template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T> inverse_jacobi_elliptic_cn_backward_backward(
    T grad_grad_x,
    T grad_grad_m,
    T gradient,
    T x,
    ...
    T h = detail::inverse_jacobi_cn_finite_diff_step<T>();

    // Compute function values at various points
    T f_center = inverse_jacobi_elliptic_cn(x, m);
    T f_x_plus = inverse_jacobi_elliptic_cn(x + h, m);
    T f_x_minus = inverse_jacobi_elliptic_cn(x - h, m);
    T f_m_plus = inverse_jacobi_elliptic_cn(x, m + h);
    T f_m_minus = inverse_jacobi_elliptic_cn(x, m - h);

    // For mixed and second partials
    T f_x_plus_m_plus = inverse_jacobi_elliptic_cn(x + h, m + h);
    T f_x_plus_m_minus = inverse_jacobi_elliptic_cn(x + h, m - h);
    T f_x_minus_m_plus = inverse_jacobi_elliptic_cn(x - h, m + h);
    T f_x_minus_m_minus = inverse_jacobi_elliptic_cn(x - h, m - h);
...
inverse_jacobi_elliptic_cn_backward_backward(
    ...
    c10::complex<T> f_center = inverse_jacobi_elliptic_cn(x, m);
    c10::complex<T> f_x_plus = inverse_jacobi_elliptic_cn(x + h_complex, m);
    c10::complex<T> f_x_minus = inverse_jacobi_elliptic_cn(x - h_complex, m);
    c10::complex<T> f_m_plus = inverse_jacobi_elliptic_cn(x, m + h_complex);
    c10::complex<T> f_m_minus = inverse_jacobi_elliptic_cn(x, m - h_complex);

    // For mixed and second partials
    c10::complex<T> f_x_plus_m_plus = inverse_jacobi_elliptic_cn(x + h_complex, m + h_complex);
    c10::complex<T> f_x_plus_m_minus = inverse_jacobi_elliptic_cn(x + h_complex, m - h_complex);
    c10::complex<T> f_x_minus_m_plus = inverse_jacobi_elliptic_cn(x - h_complex, m + h_complex);
    c10::complex<T> f_x_minus_m_minus = inverse_jacobi_elliptic_cn(x - h_complex, m - h_complex);
