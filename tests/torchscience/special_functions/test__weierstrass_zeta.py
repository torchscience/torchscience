...
def mpmath_weierstrass_zeta(z: complex, g2: float, g3: float) -> complex:
    """Reference implementation using mpmath.ellipfun('Z', ...)."""
    with mpmath.workdps(30):
        ...
        # mpmath uses 'Z' for the Weierstrass zeta function
        result = mpmath.ellipfun("Z", mp_z, g2=g2, g3=g3)
        return complex(result)


class TestWeierstrassZeta:
    """Tests for Weierstrass zeta function zeta(z; g2, g3)."""
    ...
    def test_mpmath_agreement(self):
        ...
        for z_real, z_imag, g2_val, g3_val in test_cases:
            ...
            result = torchscience.special_functions.weierstrass_zeta(z, g2, g3)
            expected = mpmath_weierstrass_zeta(z_val, g2_val, g3_val)
    ...
    def test_pole_at_zero(self):
        """Test that zeta(0) = infinity (simple pole)."""
        ...
        assert torch.isinf(result).all(), (
            f"Expected infinity at z=0, got {result}"
        )

    def test_pole_at_zero_various_invariants(self):
        """Test that zeta(0) = infinity for various g2, g3."""
        test_cases = [
            (1.0, 0.0),
            (0.0, 1.0),
            (2.0, 0.5),
        ...
        for g2_val, g3_val in test_cases:
            ...
            assert torch.isinf(result).all(), (
                f"Expected infinity at z=0 with g2={g2_val}, g3={g3_val}, got {result}"
            )

    def test_odd_function(self):
        """Test that zeta(-z) = -zeta(z) (zeta is odd)."""
    ...
    def test_odd_function_complex(self):
        ...
        torch.testing.assert_close(zeta_pos, -zeta_neg, rtol=1e-8, atol=1e-8)
    ...
    def test_odd_function_multiple_points(self):
        ...
        for z_val in z_vals:
            ...
            torch.testing.assert_close(
                zeta_pos, -zeta_neg, rtol=1e-10, atol=1e-10
            )

    def test_derivative_is_negative_p(self):
        """Test that zeta'(z) = -P(z) using finite differences."""
        z = torch.tensor([0.5], dtype=torch.float64)
        g2 = torch.tensor([1.0], dtype=torch.float64)
        g3 = torch.tensor([0.5], dtype=torch.float64)

        # Compute zeta'(z) numerically
        h = 1e-6
        z_plus = torch.tensor([0.5 + h], dtype=torch.float64)
        z_minus = torch.tensor([0.5 - h], dtype=torch.float64)

        zeta_plus = torchscience.special_functions.weierstrass_zeta(
            z_plus, g2, g3
        )
        zeta_minus = torchscience.special_functions.weierstrass_zeta(
            z_minus, g2, g3
        )
        zeta_derivative = (zeta_plus - zeta_minus) / (2 * h)

        # Compute -P(z)
        p_value = torchscience.special_functions.weierstrass_p(z, g2, g3)
        expected = -p_value

        torch.testing.assert_close(
            zeta_derivative, expected, rtol=1e-4, atol=1e-4
        )

    def test_sigma_ratio(self):
        """Test that zeta(z) = sigma'(z) / sigma(z) using finite differences."""
        z = torch.tensor([0.5], dtype=torch.float64)
        g2 = torch.tensor([1.0], dtype=torch.float64)
        g3 = torch.tensor([0.5], dtype=torch.float64)

        # Compute sigma(z) and sigma'(z) numerically
        h = 1e-6
        z_plus = torch.tensor([0.5 + h], dtype=torch.float64)
        z_minus = torch.tensor([0.5 - h], dtype=torch.float64)

        sigma_center = torchscience.special_functions.weierstrass_sigma(
            z, g2, g3
        )
        sigma_plus = torchscience.special_functions.weierstrass_sigma(
            z_plus, g2, g3
        )
        sigma_minus = torchscience.special_functions.weierstrass_sigma(
            z_minus, g2, g3
        )

        sigma_derivative = (sigma_plus - sigma_minus) / (2 * h)
        expected_zeta = sigma_derivative / sigma_center

        # Compute zeta(z) directly
        zeta_value = torchscience.special_functions.weierstrass_zeta(z, g2, g3)

        torch.testing.assert_close(
            zeta_value, expected_zeta, rtol=1e-4, atol=1e-4
        )
    ...
    def test_gradcheck(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.weierstrass_zeta,
            (z, g2, g3),
            eps=1e-5,
            atol=1e-3,
            rtol=1e-3,
    ...
    def test_laurent_series_near_zero(self):
        """Test behavior near z=0 matches Laurent series.
        ...
        # Laurent approximation: zeta(z) ~ 1/z - (g2/60)*z^3 - (g3/140)*z^5
        laurent_approx = (
            1.0 / z_val - (g2_val / 60) * z_val**3 - (g3_val / 140) * z_val**5
        )

        # For small z, should be close
        torch.testing.assert_close(
            result,
            torch.tensor([laurent_approx], dtype=torch.float64),
            rtol=1e-3,
            atol=1e-3,
        )

    def test_finite_away_from_pole(self):
        """Test that zeta is finite away from lattice points."""
        # Away from z = 0, zeta should be finite
    ...
    def test_cuda(self):
        ...
        expected = torchscience.special_functions.weierstrass_zeta(
            z_cpu, g2_cpu, g3_cpu
        )
    ...
    def test_homogeneity(self):
        ...
        zeta(tz; t^-4 g2, t^-6 g3) = t^-1 * zeta(z; g2, g3)
        """
        z = torch.tensor([0.5], dtype=torch.float64)
        g2 = torch.tensor([1.0], dtype=torch.float64)
        g3 = torch.tensor([0.5], dtype=torch.float64)
        ...
        # zeta(z; g2, g3)
        zeta_original = torchscience.special_functions.weierstrass_zeta(
            z, g2, g3
        )

        # zeta(tz; t^-4 g2, t^-6 g3)
        z_scaled = t * z
        g2_scaled = (t ** (-4)) * g2
        g3_scaled = (t ** (-6)) * g3
        zeta_scaled = torchscience.special_functions.weierstrass_zeta(
            z_scaled, g2_scaled, g3_scaled
        )

        # Should equal t^-1 * zeta(z; g2, g3)
        expected = (t ** (-1)) * zeta_original

        torch.testing.assert_close(zeta_scaled, expected, rtol=1e-5, atol=1e-5)
