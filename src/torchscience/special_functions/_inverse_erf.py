...
def inverse_erf(input: Tensor, *, out: Tensor | None = None) -> Tensor:
    """
    ...
    - ``inverse_erf(-1) = -inf``
    - ``inverse_erf(1) = inf``
    - ``inverse_erf(-x) = -inverse_erf(x)`` (odd function)
    ...
    Examples
    --------
    >>> inverse_erf(torch.tensor([0.0, 0.5, -0.5]))
    tensor([ 0.0000,  0.4769, -0.4769])

    >>> inverse_erf(torch.tensor([0.8427]))  # erf(1) ≈ 0.8427
    tensor([1.0000])
    """
    output: Tensor = torch.ops.torchscience._inverse_erf(input)
