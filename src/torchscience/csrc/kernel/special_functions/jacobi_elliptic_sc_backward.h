...
template <typename T>
inline T jacobi_sc_finite_diff_step() {
    return std::pow(std::numeric_limits<T>::epsilon(), T(1.0/3.0));
}

template <>
inline float jacobi_sc_finite_diff_step<float>() {
    return 1e-3f;
}

template <>
inline double jacobi_sc_finite_diff_step<double>() {
    return 1e-6;
}
...
template <typename T>
std::tuple<T, T> jacobi_elliptic_sc_backward(
    T gradient,
    T u,
    T m
) {
...
template <typename T>
std::tuple<c10::complex<T>, c10::complex<T>> jacobi_elliptic_sc_backward(
