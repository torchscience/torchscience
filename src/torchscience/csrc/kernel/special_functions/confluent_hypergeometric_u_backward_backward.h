...
#include "confluent_hypergeometric_u_backward.h"

namespace torchscience::kernel::special_functions {

template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T, T> confluent_hypergeometric_u_backward_backward(
  T gg_a,
  T gg_b,
  T gg_z,
  T grad,
  ...
  using real_t = detail::hypu_real_type_t<T>;

  // d/dz U(a, b, z) = -a * U(a+1, b+1, z)
  // d²/dz² U(a, b, z) = a*(a+1) * U(a+2, b+2, z)
  T dz_coef = -a;
  T d2z_coef = a * (a + T(1));

  T f_shifted = confluent_hypergeometric_u(a + T(1), b + T(1), z);
  T f_double_shifted = confluent_hypergeometric_u(a + T(2), b + T(2), z);

  // Use larger step for second derivatives (4th root of epsilon)
  real_t eps_real = std::pow(hypu_epsilon<T>(), real_t(0.25));
  T eps = T(eps_real);
  ...
  auto [da_z_plus, db_z_plus, dz_z_plus] =
    confluent_hypergeometric_u_backward(T(1), a, b, z + eps);
