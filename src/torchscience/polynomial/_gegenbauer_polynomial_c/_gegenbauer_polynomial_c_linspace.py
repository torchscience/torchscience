...
def gegenbauer_polynomial_c_linspace(
    n: int,
    ...
    """Generate n evenly spaced points in the Gegenbauer domain.
    ...
    Notes
    -----
    The default domain [-1, 1] is the natural domain for Gegenbauer polynomials.
    For non-uniform sampling optimal for integration, use
    gegenbauer_polynomial_c_points instead.

    Examples
    --------
    >>> gegenbauer_polynomial_c_linspace(5)
    tensor([-1.0000, -0.5000,  0.0000,  0.5000,  1.0000])

    >>> gegenbauer_polynomial_c_linspace(3, start=0.0, end=1.0)
