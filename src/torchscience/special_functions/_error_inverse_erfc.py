...
def error_inverse_erfc(input: Tensor, *, out: Tensor | None = None) -> Tensor:
    """
    ...
    - ``error_inverse_erfc(1) = 0``
    - ``error_inverse_erfc(0) = inf``
    - ``error_inverse_erfc(2) = -inf``
    ...
    Examples
    --------
    >>> error_inverse_erfc(torch.tensor([0.5, 1.0, 1.5]))
    tensor([ 0.4769,  0.0000, -0.4769])

    >>> error_inverse_erfc(torch.tensor([0.1, 0.9]))
    tensor([1.1631, 0.0889])
    """
    output: Tensor = torch.ops.torchscience._error_inverse_erfc(input)
