...
class TestModifiedBesselI:
    """Tests for the modified Bessel function Iₙ(z) of general order n."""
    ...
    def test_order_zero_matches_i0(self):
        """Test I₀(z) via modified_bessel_i(0, z) matches modified_bessel_i_0(z)."""
        z = torch.tensor([0.0, 0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        n = torch.zeros_like(z)
        result = torchscience.special_functions.modified_bessel_i(n, z)
        expected = torchscience.special_functions.modified_bessel_i_0(z)
        torch.testing.assert_close(result, expected, rtol=1e-10, atol=1e-10)

    def test_order_one_matches_i1(self):
        """Test I₁(z) via modified_bessel_i(1, z) matches modified_bessel_i_1(z)."""
    ...
    def test_scipy_agreement_integer_orders(self):
        """Test agreement with scipy.special.iv for integer orders."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        for order in [0, 1, 2, 3, 5, 10]:
            n = torch.full_like(z, float(order))
            result = torchscience.special_functions.modified_bessel_i(n, z)
            expected = torch.tensor(
                [scipy.special.iv(order, x.item()) for x in z],
                dtype=torch.float64,
            )
    ...
    def test_at_zero_positive_order(self):
        """Test Iₙ(0) = 0 for n != 0 integer."""
        z = torch.tensor([0.0], dtype=torch.float64)
        for order in [1, 2, 3, 5, -1, -2]:
    ...
    def test_at_zero_fractional_order(self):
        """Test Iₙ(0) = 0 for n > 0 non-integer."""
        z = torch.tensor([0.0], dtype=torch.float64)
    ...
    def test_positive_for_positive_z(self):
        """Test Iₙ(z) > 0 for z > 0 and real n."""
        n = torch.tensor([0.0, 1.0, 2.0, 0.5, 1.5], dtype=torch.float64)
        z = torch.tensor([0.5, 1.0, 2.0, 3.0, 5.0], dtype=torch.float64)
        result = torchscience.special_functions.modified_bessel_i(n, z)
        assert torch.all(result > 0), "Iₙ(z) should be positive for z > 0"
    ...
    def test_negative_integer_order_symmetry(self):
        """Test I_{-n}(z) = Iₙ(z) for integer n."""
        z = torch.tensor([1.0, 2.0, 5.0], dtype=torch.float64)
        for order in [1, 2, 3, 4, 5]:
            n_pos = torch.full_like(z, float(order))
            n_neg = torch.full_like(z, float(-order))
            result_pos = torchscience.special_functions.modified_bessel_i(
                n_pos, z
            )
            result_neg = torchscience.special_functions.modified_bessel_i(
                n_neg, z
            )
            torch.testing.assert_close(
                result_neg,
    ...
    def test_recurrence_relation(self):
        """Test recurrence: Iₙ₋₁(z) - Iₙ₊₁(z) = (2n/z) * Iₙ(z)."""
        ...
        for order in [1.0, 2.0, 3.0, 0.5, 1.5]:
            ...
            i_n = torchscience.special_functions.modified_bessel_i(n, z)
            i_nm1 = torchscience.special_functions.modified_bessel_i(
                n_minus, z
            )
            i_np1 = torchscience.special_functions.modified_bessel_i(n_plus, z)
    ...
    def test_another_recurrence(self):
        """Test recurrence: Iₙ₋₁(z) + Iₙ₊₁(z) = 2 * dIₙ/dz.

        This is equivalent to: Iₙ₊₁(z) = Iₙ₋₁(z) - (2n/z) * Iₙ(z) for forward recurrence.
        """
        z = torch.tensor([1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
    ...
    def test_gradcheck_z_integer_order(self):
        ...
        def func(z_):
            return torchscience.special_functions.modified_bessel_i(n, z_)
    ...
    def test_gradcheck_both_inputs(self):
        ...
        def func(n_, z_):
            return torchscience.special_functions.modified_bessel_i(n_, z_)
    ...
    def test_gradient_z_identity(self):
        ...
        i_np1 = torchscience.special_functions.modified_bessel_i(
            n_plus, z.detach()
        )
        expected = (i_nm1 + i_np1) / 2
    ...
    def test_complex_near_real_accuracy(self):
        ...
        expected = torch.tensor(
            [
                scipy.special.iv(n[i].real.item(), z[i].item())
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real Iₙ."""
        ...
        result_real = torchscience.special_functions.modified_bessel_i(
            n_real, z_real
        )
        result_complex = torchscience.special_functions.modified_bessel_i(
            n_complex, z_complex
        )
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.modified_bessel_i(n, z)
    ...
    def test_vmap(self):
        ...
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.float64)
        z = torch.randn(5, 3, dtype=torch.float64).abs() + 0.5  # positive z

        def fn(z_row):
            return torchscience.special_functions.modified_bessel_i(n, z_row)

        result = torch.vmap(fn)(z)
        expected = torchscience.special_functions.modified_bessel_i(
            n.unsqueeze(0), z
        )
        torch.testing.assert_close(result, expected)
    ...
    def test_compile(self):
        ...
        compiled_fn = torch.compile(
            torchscience.special_functions.modified_bessel_i
        )
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.float64)
        z = torch.randn(3, dtype=torch.float64).abs() + 0.5
    ...
    def test_compile_with_autograd(self):
        ...
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.float64)
        z = (torch.randn(3, dtype=torch.float64).abs() + 0.5).requires_grad_(
            True
        )
        result = compiled_fn(n, z)
        result.sum().backward()
        ...
        expected = torchscience.special_functions.modified_bessel_i(n, z2)
    ...
    def test_large_z_exponential_growth(self):
        """Test large z values - Iₙ(z) grows exponentially."""
        n = torch.tensor([0.0, 1.0, 2.0, 5.0], dtype=torch.float64)
        z = torch.tensor([20.0, 30.0, 40.0, 50.0], dtype=torch.float64)
        result = torchscience.special_functions.modified_bessel_i(
            n.unsqueeze(1), z
        )
        expected = torch.tensor(
            [[scipy.special.iv(ni.item(), zi.item()) for zi in z] for ni in n],
            dtype=torch.float64,
        )
        # Relaxed tolerance for large values
        torch.testing.assert_close(result, expected, rtol=1e-6, atol=1e-6)

    def test_large_order(self):
        """Test large order values."""
        n = torch.tensor([10.0, 20.0, 30.0], dtype=torch.float64)
        z = torch.tensor([15.0, 25.0, 35.0], dtype=torch.float64)
        result = torchscience.special_functions.modified_bessel_i(n, z)
        expected = torch.tensor(
            [
                scipy.special.iv(n[i].item(), z[i].item())
                for i in range(len(z))
            ],
            dtype=torch.float64,
        )
    ...
    def test_negative_z_integer_order(self):
        """Test Iₙ(-z) = (-1)^n * Iₙ(z) for integer n."""
        z = torch.tensor([1.0, 2.0, 3.0], dtype=torch.float64)
        for order in [0, 1, 2, 3]:
            n = torch.full_like(z, float(order))
            result_pos = torchscience.special_functions.modified_bessel_i(n, z)
            result_neg = torchscience.special_functions.modified_bessel_i(
                n, -z
            )
