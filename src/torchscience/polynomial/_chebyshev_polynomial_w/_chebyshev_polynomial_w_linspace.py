...
def chebyshev_polynomial_w_linspace(
    n: int,
    ...
    """Generate n evenly spaced points in the Chebyshev W domain.
    ...
    Notes
    -----
    The default domain [-1, 1] is the natural domain for Chebyshev W polynomials.
    For non-uniform sampling optimal for integration, use
    chebyshev_polynomial_w_points instead.

    Examples
    --------
    >>> chebyshev_polynomial_w_linspace(5)
    tensor([-1.0000, -0.5000,  0.0000,  0.5000,  1.0000])

    >>> chebyshev_polynomial_w_linspace(3, start=0.0, end=1.0)
