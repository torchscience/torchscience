...
T weber_e_second_derivative_n2(T n, T z) {
    ...
    for (int j = 0; j < 10; ++j) {
        ...
        T integrand_pos = -theta_pos * theta_pos * std::sin(arg_pos);
        result += weights[j] * integrand_pos;

        T t_neg = -nodes[j];
        T theta_neg = half_pi * (t_neg + T(1));
        T arg_neg = n * theta_neg - z * std::sin(theta_neg);
        T integrand_neg = -theta_neg * theta_neg * std::sin(arg_neg);
...
T weber_e_second_derivative_nz(T n, T z) {
    ...
    for (int j = 0; j < 10; ++j) {
        ...
        T integrand_pos = theta_pos * sin_theta_pos * std::sin(arg_pos);
        result += weights[j] * integrand_pos;
        ...
        T integrand_neg = theta_neg * sin_theta_neg * std::sin(arg_neg);
        result += weights[j] * integrand_neg;
    }
    return result * half_pi / pi;
}
...
std::tuple<T, T, T> weber_e_backward_backward(
    ...
    // First derivatives
    T dfdn = weber_e_derivative_n(n, z);
    T e_nu_minus_1 = weber_e(n - T(1), z);
    T e_nu_plus_1 = weber_e(n + T(1), z);
    T dfdz = T(0.5) * (e_nu_minus_1 - e_nu_plus_1);

    // Second derivatives
    T d2f_dn2 = weber_e_second_derivative_n2(n, z);
    T d2f_dndz = weber_e_second_derivative_nz(n, z);

    T e_nu = weber_e(n, z);
    T e_nu_minus_2 = weber_e(n - T(2), z);
    T e_nu_plus_2 = weber_e(n + T(2), z);
    T d2f_dz2 = T(0.25) * (e_nu_minus_2 - T(2) * e_nu + e_nu_plus_2);
...
std::tuple<c10::complex<T>, c10::complex<T>, c10::complex<T>> weber_e_backward_backward(
    ...
    c10::complex<T> dfdn = weber_e_derivative_n(n, z);
    c10::complex<T> e_nu_minus_1 = weber_e(n - one, z);
    c10::complex<T> e_nu_plus_1 = weber_e(n + one, z);
    c10::complex<T> dfdz = half * (e_nu_minus_1 - e_nu_plus_1);

    // Second derivatives - use recurrence for z derivatives
    c10::complex<T> e_nu = weber_e(n, z);
    c10::complex<T> e_nu_minus_2 = weber_e(n - two, z);
    c10::complex<T> e_nu_plus_2 = weber_e(n + two, z);
    c10::complex<T> d2f_dz2 = quarter * (e_nu_minus_2 - two * e_nu + e_nu_plus_2);

    // For mixed derivatives, use finite differences on the derivative
    c10::complex<T> eps(T(1e-6), T(0));
    c10::complex<T> dfdn_plus = weber_e_derivative_n(n + eps, z);
    c10::complex<T> dfdn_minus = weber_e_derivative_n(n - eps, z);
    c10::complex<T> d2f_dn2 = (dfdn_plus - dfdn_minus) / (two * eps);
    c10::complex<T> d2f_dndz = (weber_e_derivative_n(n, z + eps) - weber_e_derivative_n(n, z - eps)) / (two * eps);
