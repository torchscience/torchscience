...
// Generate 1-D blue noise: PSD ~ f (alpha = +1).
//
// Spectral mirror of pink noise: emphasizes high frequencies.
//
// Algorithm:
...
//      |X(f)|^2 ~ f.
//   4. The DC bin is naturally zero because sqrt(0) = 0; this makes the
//      time-domain output exactly zero-mean (no manual zeroing required).
//   5. Inverse FFT.
...
inline void blue_noise(
    ...
    TORCH_CHECK(out.dim() == 1, "blue_noise: internal tensor must be 1-D");
    TORCH_CHECK(out.is_cpu(), "blue_noise: CPU tensor expected");
    ...
    auto compute_float = [&](at::Tensor& dst) {
        ...
        at::Tensor filter_amp = freqs.abs().sqrt();
