...
namespace torchscience::sparse::csr::cuda::integral_transform {

/**
 * Sparse CSR CUDA implementation of inverse Hilbert transform.
 */
inline at::Tensor inverse_hilbert_transform(
    ...
    TORCH_CHECK(
        ...
        "inverse_hilbert_transform (SparseCsrCUDA) expects sparse CSR CUDA tensor"
    );
...
/**
 * Backward pass for sparse CSR CUDA inverse Hilbert transform.
 */
inline at::Tensor inverse_hilbert_transform_backward(
    ...
    TORCH_CHECK(
        input.layout() == at::kSparseCsr && input.is_cuda(),
        "inverse_hilbert_transform_backward (SparseCsrCUDA) expects sparse CSR CUDA tensor for input"
    );
...
/**
 * Double backward pass for sparse CSR CUDA inverse Hilbert transform.
 */
...
TORCH_LIBRARY_IMPL(torchscience, SparseCsrCUDA, module) {
    module.impl(
        ...
        &torchscience::sparse::csr::cuda::integral_transform::inverse_hilbert_transform
    );
    module.impl(
        "inverse_hilbert_transform_backward",
        &torchscience::sparse::csr::cuda::integral_transform::inverse_hilbert_transform_backward
    );
    module.impl(
        "inverse_hilbert_transform_backward_backward",
        &torchscience::sparse::csr::cuda::integral_transform::inverse_hilbert_transform_backward_backward
    );
}
