...
class TestFresnelC:
    ...
    def test_zero(self):
        """Test C(0) = 0."""
        result = torchscience.special_functions.fresnel_c(
    ...
    def test_nan(self):
        """Test C(nan) = nan."""
        assert torchscience.special_functions.fresnel_c(
    ...
    def test_scipy_agreement(self):
        ...
        result = torchscience.special_functions.fresnel_c(z)
        # scipy.special.fresnel returns (S, C), so we take [1]
        expected = torch.tensor(
            [scipy.special.fresnel(zi.item())[1] for zi in z],
            dtype=torch.float64,
        )
    ...
    def test_odd_symmetry(self):
        """Test odd symmetry: C(-z) = -C(z)."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0], dtype=torch.float64)
        result_pos = torchscience.special_functions.fresnel_c(z)
        result_neg = torchscience.special_functions.fresnel_c(-z)
        torch.testing.assert_close(
            result_neg, -result_pos, rtol=1e-14, atol=1e-14
        )

    def test_asymptotic_value(self):
        """Test that C(z) approaches 0.5 for large positive z."""
    ...
    def test_known_value(self):
        """Test known value: C(1) from tables."""
        result = torchscience.special_functions.fresnel_c(
    ...
    def test_gradcheck(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.fresnel_c, z, eps=1e-6
        )
    ...
    def test_gradient_formula(self):
        """Verify gradient formula: d/dz C(z) = cos(pi*z^2/2)."""
        import math
        ...
        y = torchscience.special_functions.fresnel_c(z)
        grad = torch.autograd.grad(y.sum(), z)[0]

        # Expected: cos(pi*z^2/2)
        expected = torch.cos(math.pi * z.detach() ** 2 / 2)
        torch.testing.assert_close(grad, expected, rtol=1e-8, atol=1e-10)

    def test_gradient_at_zero(self):
        """Test gradient at z=0: d/dz C(0) = cos(0) = 1."""
        z = torch.tensor([0.0], dtype=torch.float64, requires_grad=True)
        y = torchscience.special_functions.fresnel_c(z)
        grad = torch.autograd.grad(y.sum(), z)[0]
        assert grad.item() == pytest.approx(1.0, abs=1e-10)
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.fresnel_c(z)
    ...
    def test_complex_input(self):
        ...
        real_result = torchscience.special_functions.fresnel_c(
    ...
    def test_complex_gradcheck(self):
        ...
        # The derivative of C(z) is cos(pi*z^2/2)
        torch.autograd.gradcheck(
            torchscience.special_functions.fresnel_c,
            (z,),
            eps=1e-6,
            atol=1e-3,
            rtol=1e-3,
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(torchscience.special_functions.fresnel_c)(z)
        expected = torchscience.special_functions.fresnel_c(z)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.fresnel_c)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.fresnel_c(z2)
    ...
    def test_broadcasting(self):
        ...
        combined = z1 + z2
        result = torchscience.special_functions.fresnel_c(combined)
