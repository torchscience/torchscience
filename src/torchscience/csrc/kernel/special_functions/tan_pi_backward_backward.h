...
template <typename T>
std::tuple<T, T> tan_pi_backward_backward(T gradient_gradient, T g, T x) {
  ...
  return std::make_tuple(gradient_gradient * pi * sec2_pi_x, gradient_gradient * g * T(2) * pi * pi * tan_pi(x) * sec2_pi_x);
}
...
std::tuple<c10::complex<T>, c10::complex<T>> tan_pi_backward_backward(
  ...
  c10::complex<T> g_z = gradient_gradient * g * T(2) * pi * pi * tan_pi(z) * sec2_pi_z;
