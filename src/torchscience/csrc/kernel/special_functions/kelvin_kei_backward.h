...
//
template <typename T>
C10_HOST_DEVICE T kelvin_kei_derivative(T x) {
...
template <typename T>
C10_HOST_DEVICE T kelvin_kei_backward(T grad_output, T x) {
    return grad_output * detail::kelvin_kei_derivative(x);
}

// Complex backward
template <typename T>
C10_HOST_DEVICE c10::complex<T> kelvin_kei_backward(c10::complex<T> grad_output, c10::complex<T> x) {
