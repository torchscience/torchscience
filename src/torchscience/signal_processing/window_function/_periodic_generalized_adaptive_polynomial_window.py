...
def periodic_generalized_adaptive_polynomial_window(
    n: int,
    ...
    *,
    ...
    """
    Generalized adaptive polynomial window function (periodic).

    Computes a periodic generalized adaptive polynomial window of length n.
    The periodic version is designed for spectral analysis where the window
    will be used with DFT/FFT.

    Mathematical Definition
    -----------------------
    The periodic generalized adaptive polynomial window is defined as:

        w[k] = (1 - |x[k]|^alpha)^beta

    where x[k] = 2k / n - 1 is the normalized position in [-1, 1)
    for k = 0, 1, ..., n-1.
    ...
    Special Cases
    -------------
    - alpha=1, beta=1: Triangular-like window (periodic)
    - alpha=2, beta=1: Welch-like window (periodic)
    - alpha=2, beta=0.5: Cosine-like window (periodic)
    ...
    See Also
    --------
    generalized_adaptive_polynomial_window : Symmetric version.
    periodic_welch_window : Equivalent to periodic generalized adaptive
        polynomial window with alpha=2, beta=1.
    """
    if n < 0:
        raise ValueError(
            f"periodic_generalized_adaptive_polynomial_window: n must be "
    ...
    return (
        torch.ops.torchscience.periodic_generalized_adaptive_polynomial_window(
