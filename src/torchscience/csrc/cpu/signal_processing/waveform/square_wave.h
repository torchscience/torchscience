...
#include "../../../kernel/signal_processing/waveform/square_wave.h"

namespace torchscience::cpu::signal_processing::waveform {

inline at::Tensor square_wave(
  ...
  TORCH_CHECK(
      ...
      "square_wave: Exactly one of n or t must be provided");

  if (n.has_value()) {
    TORCH_CHECK(n.value() >= 0, "square_wave: n must be non-negative, got ", n.value());
  }
  ...
  if (t.has_value()) {
    ...
    TORCH_CHECK(sample_rate > 0, "square_wave: sample_rate must be positive");
    auto options = at::TensorOptions()
        .dtype(dtype.value_or(at::kFloat))
        .device(device.value_or(at::kCPU));
  ...
  auto duty_exp = duty.expand(broadcast_shape).contiguous();
  ...
  AT_DISPATCH_FLOATING_TYPES(output.scalar_type(), "square_wave_cpu", [&] {
    ...
    auto duty_data = duty_exp.data_ptr<scalar_t>();
    ...
    at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
      for (int64_t b = begin; b < end; ++b) {
        ...
        scalar_t dt = duty_data[b];

        for (int64_t i = 0; i < n_samples; ++i) {
          out_data[b * n_samples + i] = kernel::square_wave_kernel(
              time_data[i], freq, amp, ph, dt);
        }
      }
    });
  });
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
  m.impl("square_wave", &torchscience::cpu::signal_processing::waveform::square_wave);
}
