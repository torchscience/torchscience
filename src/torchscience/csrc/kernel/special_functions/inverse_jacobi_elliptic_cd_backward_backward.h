...
#include "inverse_jacobi_elliptic_cd_backward.h"

namespace torchscience::kernel::special_functions {

// Second-order backward pass for inverse Jacobi elliptic function arccd(x, m)
//
// Uses numerical finite differences for second derivatives.
// Returns gradients w.r.t. (grad_output, x, m)

template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T> inverse_jacobi_elliptic_cd_backward_backward(
    T grad_grad_x,
    T grad_grad_m,
    T gradient,
    T x,
    ...
    T f_center = inverse_jacobi_elliptic_cd(x, m);
    ...
    T f_x_plus_m_plus = inverse_jacobi_elliptic_cd(x + h, m + h);
    T f_x_plus_m_minus = inverse_jacobi_elliptic_cd(x + h, m - h);
    T f_x_minus_m_plus = inverse_jacobi_elliptic_cd(x - h, m + h);
    T f_x_minus_m_minus = inverse_jacobi_elliptic_cd(x - h, m - h);
...
inverse_jacobi_elliptic_cd_backward_backward(
    ...
    c10::complex<T> f_center = inverse_jacobi_elliptic_cd(x, m);
    ...
    // For mixed and second partials
    c10::complex<T> f_x_plus_m_plus = inverse_jacobi_elliptic_cd(x + h_complex, m + h_complex);
    c10::complex<T> f_x_plus_m_minus = inverse_jacobi_elliptic_cd(x + h_complex, m - h_complex);
    c10::complex<T> f_x_minus_m_plus = inverse_jacobi_elliptic_cd(x - h_complex, m + h_complex);
    c10::complex<T> f_x_minus_m_minus = inverse_jacobi_elliptic_cd(x - h_complex, m - h_complex);
