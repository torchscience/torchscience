"""Antiderivative of Chebyshev series."""
...
def chebyshev_t_antiderivative(
    a: ChebyshevT, order: int = 1, constant: float = 0.0
) -> ChebyshevT:
    ...
    Examples
    --------
    >>> a = chebyshev_t(torch.tensor([1.0]))  # constant 1 = T_0
    ...
    for i in range(order):
        ...
        temp = ChebyshevT(coeffs=i_coeffs)
        x_zero = torch.zeros((), dtype=coeffs.dtype, device=coeffs.device)
        val_at_zero = chebyshev_t_evaluate(temp, x_zero)
