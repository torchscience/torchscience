...
#include "jacobi_elliptic_dn_backward.h"

namespace torchscience::kernel::special_functions {

// Second-order backward pass for Jacobi elliptic function dn(u, m)
//
...
T compute_d2dn_du2(T u, T m) {
    ...
    T dn_center = jacobi_elliptic_dn(u, m);
    T dn_minus = jacobi_elliptic_dn(u - h, m);

    return (dn_plus - T(2) * dn_center + dn_minus) / (h * h);
}
...
T compute_d2dn_dm2(T u, T m) {
    ...
    if (m < h) {
        ...
        T dn_2 = jacobi_elliptic_dn(u, m + T(2) * h);
        return (dn_2 - T(2) * dn_1 + dn_0) / (h * h);
    } else if (m > T(1) - h) {
...
T compute_d2dn_dudm(T u, T m) {
    ...
    // Mixed partial: d^2(dn)/du/dm
    // Use central differences: (dn(u+h,m+h) - dn(u+h,m-h) - dn(u-h,m+h) + dn(u-h,m-h)) / (4*h^2)
    ...
    T f_pp = jacobi_elliptic_dn(u + h, m_plus);
    T f_pm = jacobi_elliptic_dn(u + h, m_minus);
    T f_mp = jacobi_elliptic_dn(u - h, m_plus);
    T f_mm = jacobi_elliptic_dn(u - h, m_minus);
...
c10::complex<T> compute_d2dn_du2(c10::complex<T> u, c10::complex<T> m) {
    ...
    c10::complex<T> dn_center = jacobi_elliptic_dn(u, m);
    c10::complex<T> dn_minus = jacobi_elliptic_dn(u - h_c, m);

    return (dn_plus - two * dn_center + dn_minus) / (h_c * h_c);
}
...
c10::complex<T> compute_d2dn_dudm(c10::complex<T> u, c10::complex<T> m) {
    ...
    c10::complex<T> f_pp = jacobi_elliptic_dn(u + h_c, m + h_c);
    c10::complex<T> f_pm = jacobi_elliptic_dn(u + h_c, m - h_c);
    c10::complex<T> f_mp = jacobi_elliptic_dn(u - h_c, m + h_c);
    c10::complex<T> f_mm = jacobi_elliptic_dn(u - h_c, m - h_c);
...
std::tuple<T, T, T> jacobi_elliptic_dn_backward_backward(
    ...
    //
    // d(grad_u)/d(grad) = ddn/du
    // d(grad_u)/du = grad * d^2(dn)/du^2
    // d(grad_u)/dm = grad * d^2(dn)/du/dm
    //
    // d(grad_m)/d(grad) = ddn/dm
    // d(grad_m)/du = grad * d^2(dn)/dm/du
    // d(grad_m)/dm = grad * d^2(dn)/dm^2

    T ddn_du = detail::compute_ddn_du(u, m);
    T ddn_dm = detail::compute_ddn_dm(u, m);

    T d2dn_du2 = detail::compute_d2dn_du2(u, m);
    T d2dn_dm2 = detail::compute_d2dn_dm2(u, m);
    T d2dn_dudm = detail::compute_d2dn_dudm(u, m);

    // Gradient w.r.t. grad
    T grad_grad = gg_u * ddn_du + gg_m * ddn_dm;

    // Gradient w.r.t. u
    T grad_u_out = gg_u * grad * d2dn_du2 + gg_m * grad * d2dn_dudm;

    // Gradient w.r.t. m
    T grad_m_out = gg_u * grad * d2dn_dudm + gg_m * grad * d2dn_dm2;
...
std::tuple<c10::complex<T>, c10::complex<T>, c10::complex<T>> jacobi_elliptic_dn_backward_backward(
    ...
    c10::complex<T> ddn_du = detail::compute_ddn_du(u, m);
    c10::complex<T> ddn_dm = detail::compute_ddn_dm(u, m);

    c10::complex<T> d2dn_du2 = detail::compute_d2dn_du2(u, m);
    c10::complex<T> d2dn_dm2 = detail::compute_d2dn_dm2(u, m);
    c10::complex<T> d2dn_dudm = detail::compute_d2dn_dudm(u, m);

    // For complex, use Wirtinger derivatives (conjugate the derivative)
    c10::complex<T> grad_grad = gg_u * std::conj(ddn_du) + gg_m * std::conj(ddn_dm);

    c10::complex<T> grad_u_out = gg_u * grad * std::conj(d2dn_du2) + gg_m * grad * std::conj(d2dn_dudm);

    c10::complex<T> grad_m_out = gg_u * grad * std::conj(d2dn_dudm) + gg_m * grad * std::conj(d2dn_dm2);
