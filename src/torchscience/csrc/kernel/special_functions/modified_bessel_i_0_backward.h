...
template <typename T>
C10_HOST_DEVICE T modified_bessel_i_0_backward(T grad_output, T z) {
    return grad_output * modified_bessel_i_1(z);
}

// Complex backward: PyTorch expects grad * conj(derivative) for holomorphic functions
template <typename T>
C10_HOST_DEVICE c10::complex<T> modified_bessel_i_0_backward(c10::complex<T> grad_output, c10::complex<T> z) {
