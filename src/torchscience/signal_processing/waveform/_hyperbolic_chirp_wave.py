...
def hyperbolic_chirp_wave(
    n: Optional[int] = None,
    t: Optional[Tensor] = None,
    *,
    f0: Union[float, Tensor] = 10.0,
    f1: Union[float, Tensor] = 1.0,
    t1: float = 1.0,
    ...
    """
    Hyperbolic chirp (frequency sweep) waveform generator.

    Generates a chirp signal where the instantaneous frequency varies
    hyperbolically from f0 to f1 over the time interval [0, t1]:

        f(t) = alpha / (beta * t + gamma)

    where alpha = f0 * f1 * t1, beta = f0 - f1, gamma = f1 * t1.

    The output signal is:

        y(t) = amplitude * cos(phi(t) + phase)

    where the instantaneous phase is:

        phi(t) = 2*pi * f0 * f1 * t1 / (f1 - f0) * ln(|1 - t * (f1 - f0) / (f1 * t1)|)
    ...
    f0 : float or Tensor
        ...
        Default is 10.0 (typically starts high for hyperbolic sweeps).
    f1 : float or Tensor
        Ending frequency in Hz (at time t1). Tensor enables batched generation.
        Default is 1.0 (typically ends low for hyperbolic sweeps).
    t1 : float
    ...
    Examples
    --------
    Generate a basic hyperbolic chirp from 10 Hz down to 1 Hz:

    >>> hyperbolic_chirp_wave(n=1000, f0=10.0, f1=1.0, sample_rate=1000.0)

    Generate batched chirps with different starting frequencies:

    >>> f0 = torch.tensor([10.0, 20.0, 30.0])
    >>> hyperbolic_chirp_wave(n=1000, f0=f0, f1=1.0, sample_rate=1000.0)  # shape (3, 1000)

    Use explicit time tensor:

    >>> t = torch.linspace(0, 1, 1000)
    >>> hyperbolic_chirp_wave(t=t, f0=10.0, f1=1.0)

    Notes
    -----
    The hyperbolic chirp is useful when equal time is desired for each octave
    of frequency change. Unlike linear chirps, the frequency changes slowly
    at first and then more rapidly (or vice versa).

    The scipy.signal.chirp function provides similar capability with
    method='hyperbolic'.

    Note that for hyperbolic chirps, f0 and f1 must have the same sign and
    the argument to the logarithm must remain positive throughout the
    sweep interval.
    """
    ...
    result = torch.ops.torchscience.hyperbolic_chirp_wave(
        n,
        t,
        f0=f0,
        f1=f1,
