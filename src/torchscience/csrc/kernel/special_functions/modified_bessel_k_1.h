#pragma once

#include <cmath>
#include <limits>
#include <c10/util/complex.h>
#include "chebyshev_polynomial_t_series_evaluate.h"
#include "modified_bessel_i_1.h"

namespace torchscience::kernel::special_functions {

namespace detail {

// Cephes coefficients for x * (K1(x) - log(x/2) * I1(x)) in interval [0, 2]
// Source: Cephes Math Library (Stephen L. Moshier)
constexpr double k1_A[] = {
    -7.02386347938628759343E-18,
    -2.42744985051936593393E-15,
    -6.66690169419932900609E-13,
    -1.41148839263352776110E-10,
    -2.21338763073472585583E-8,
    -2.43340614156596823496E-6,
    -1.73028895751305206302E-4,
    -6.97572385963986435018E-3,
    -1.22611180822657148235E-1,
    -3.53155960776544875667E-1,
     1.52530022733894777053E0
};

// Cephes coefficients for exp(x) * sqrt(x) * K1(x) in interval [2, infinity)
constexpr double k1_B[] = {
    -5.75674448366501715755E-18,  1.79405087314755922667E-17,
    -5.68946255844285935196E-17,  1.83809354436663880070E-16,
    -6.05704724837331885336E-16,  2.03870316562433424052E-15,
    -7.01983709041831346144E-15,  2.47715442448130437068E-14,
    -8.97670518232499435011E-14,  3.34841966607842919884E-13,
    -1.28917396095102890680E-12,  5.13963967348173025100E-12,
    -2.12996783842756842877E-11,  9.21831518760500529508E-11,
    -4.19035475934189648750E-10,  2.01504975519703286596E-9,
    -1.03457624656780970260E-8,   5.74108412545004946722E-8,
    -3.50196060308781257119E-7,   2.40648494783721712015E-6,
    -1.93619797416608296024E-5,   1.95215518471351631108E-4,
    -2.85781685962277938680E-3,   1.03923736576817238437E-1,
     2.72062619048444266945E0
};

} // namespace detail

template <typename T>
T modified_bessel_k_1(T x) {
    // Handle special values
    if (std::isnan(x)) {
        return std::numeric_limits<T>::quiet_NaN();
    }
    if (x <= T(0)) {
        // K1 is only defined for x > 0
        if (x == T(0)) {
            return std::numeric_limits<T>::infinity();
        }
        return std::numeric_limits<T>::quiet_NaN();
    }
    if (std::isinf(x)) {
        // K1(+inf) = 0 (exponential decay)
        return T(0);
    }

    if (x <= T(2.0)) {
        // Small argument: K1(x) = log(x/2)*I1(x) + (1/x)*P(x^2 - 2)
        // where P is the Chebyshev expansion
        T y = x * x - T(2.0);
        T result = chebyshev_polynomial_t_series_evaluate(y, detail::k1_A, 11);
        // The expansion gives x * (K1(x) - log(x/2)*I1(x)), so divide by x
        result = result / x + std::log(x / T(2.0)) * modified_bessel_i_1(x);
        return result;
    } else {
        // Large argument: K1(x) = exp(-x) / sqrt(x) * Q(8/x - 2)
        T y = T(8.0) / x - T(2.0);
        T result = chebyshev_polynomial_t_series_evaluate(y, detail::k1_B, 25);
        return result * std::exp(-x) / std::sqrt(x);
    }
}

// Complex version
template <typename T>
c10::complex<T> modified_bessel_k_1(c10::complex<T> z) {
    T mag = std::abs(z);

    // Handle z = 0
    if (mag == T(0)) {
        return c10::complex<T>(std::numeric_limits<T>::infinity(), T(0));
    }

    if (mag <= T(2.0)) {
        // Small argument: K1(z) = log(z/2)*I1(z) + (1/z)*P(z^2 - 2)
        c10::complex<T> y = z * z - c10::complex<T>(T(2.0), T(0));
        c10::complex<T> result = chebyshev_polynomial_t_series_evaluate(y, detail::k1_A, 11);
        // log(z/2) for complex z
        c10::complex<T> ln_z_half = std::log(z / c10::complex<T>(T(2.0), T(0)));
        result = result / z + ln_z_half * modified_bessel_i_1(z);
        return result;
    } else {
        // Large argument: K1(z) = exp(-z) / sqrt(z) * Q(8/z - 2)
        c10::complex<T> y = c10::complex<T>(T(8.0), T(0)) / z - c10::complex<T>(T(2.0), T(0));
        c10::complex<T> result = chebyshev_polynomial_t_series_evaluate(y, detail::k1_B, 25);
        return result * std::exp(-z) / std::sqrt(z);
    }
}

} // namespace torchscience::kernel::special_functions
