...
class TestSphericalBesselY:
    """Tests for the spherical Bessel function y_n(z) of general order n."""
    ...
    def test_order_zero_matches_y0(self):
        """Test y_0(z) via spherical_bessel_y(0, z) matches spherical_bessel_y_0(z)."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        n = torch.zeros_like(z)
        result = torchscience.special_functions.spherical_bessel_y(n, z)
        expected = torchscience.special_functions.spherical_bessel_y_0(z)
        torch.testing.assert_close(result, expected, rtol=1e-10, atol=1e-10)

    def test_order_one_matches_y1(self):
        """Test y_1(z) via spherical_bessel_y(1, z) matches spherical_bessel_y_1(z)."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        n = torch.ones_like(z)
        result = torchscience.special_functions.spherical_bessel_y(n, z)
    ...
    def test_scipy_agreement_integer_orders(self):
        """Test agreement with scipy.special.spherical_yn for integer orders."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        for order in [0, 1, 2, 3, 5, 10]:
            n = torch.full_like(z, float(order))
            result = torchscience.special_functions.spherical_bessel_y(n, z)
            expected = torch.tensor(
                [scipy.special.spherical_yn(order, x.item()) for x in z],
                dtype=torch.float64,
            )
    ...
    def test_scipy_agreement_half_order(self):
        ...
        for order in [0.5, 1.5, 2.5]:
            ...
            result = torchscience.special_functions.spherical_bessel_y(n, z)
            # For half-integer order, use Bessel Y relation
            nu = order + 0.5
            expected = torch.tensor(
                [
                    math.sqrt(math.pi / (2 * x.item()))
                    * scipy.special.yv(nu, x.item())
                    for x in z
                ],
                dtype=torch.float64,
            )
    ...
    def test_at_zero_singular(self):
        """Test y_n(0) = -infinity for all n (singular at origin)."""
    ...
    def test_nan_input(self):
        """Test y_n(NaN) = NaN and y_NaN(z) = NaN."""
    ...
    def test_recurrence_relation(self):
        """Test recurrence: y_{n-1}(z) + y_{n+1}(z) = (2n+1)/z * y_n(z)."""
        ...
        for order in [1.0, 2.0, 3.0, 0.5, 1.5]:
            ...
            y_n = torchscience.special_functions.spherical_bessel_y(n, z)
            y_nm1 = torchscience.special_functions.spherical_bessel_y(
                n_minus, z
            )
            y_np1 = torchscience.special_functions.spherical_bessel_y(
                n_plus, z
            )
    ...
    def test_negcos_identity_n0(self):
    ...
    def test_gradcheck_z_integer_order(self):
        ...
        def func(z_):
            return torchscience.special_functions.spherical_bessel_y(n, z_)
    ...
    def test_gradcheck_both_inputs(self):
        ...
        def func(n_, z_):
            return torchscience.special_functions.spherical_bessel_y(n_, z_)
    ...
    def test_gradient_z_identity(self):
        """Verify d/dz y_n(z) = (n/z)*y_n(z) - y_{n+1}(z)."""
        n = torch.tensor([1.0, 2.0, 0.5, 1.5], dtype=torch.float64)
        z = torch.tensor(
            [1.0, 2.0, 3.0, 4.0], dtype=torch.float64, requires_grad=True
        )

        y = torchscience.special_functions.spherical_bessel_y(n, z)
        grad = torch.autograd.grad(y.sum(), z)[0]

        y_n = torchscience.special_functions.spherical_bessel_y(n, z.detach())
        y_np1 = torchscience.special_functions.spherical_bessel_y(
            n + 1, z.detach()
        )
        expected = (n / z.detach()) * y_n - y_np1
    ...
    def test_complex_near_real_accuracy(self):
        ...
        expected = torch.tensor(
            [
                scipy.special.spherical_yn(int(n[i].real.item()), z[i].item())
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real y_n."""
        ...
        result_real = torchscience.special_functions.spherical_bessel_y(
            n_real, z_real
        )
        result_complex = torchscience.special_functions.spherical_bessel_y(
            n_complex, z_complex
        )
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.spherical_bessel_y(n, z)
    ...
    def test_vmap(self):
        ...
        def fn(z_row):
            return torchscience.special_functions.spherical_bessel_y(n, z_row)

        result = torch.vmap(fn)(z)
        expected = torchscience.special_functions.spherical_bessel_y(
            n.unsqueeze(0), z
        )
        torch.testing.assert_close(result, expected)
    ...
    def test_compile(self):
        ...
        compiled_fn = torch.compile(
            torchscience.special_functions.spherical_bessel_y
        )
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.float64)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.spherical_bessel_y(n, z2)
    ...
    def test_large_z(self):
        ...
        result = torchscience.special_functions.spherical_bessel_y(
            n.unsqueeze(1), z
        )
        expected = torch.tensor(
            [
                [
                    scipy.special.spherical_yn(int(ni.item()), zi.item())
                    for zi in z
                ]
                for ni in n
            ],
    ...
    def test_asymptotic_behavior(self):
        """Test asymptotic behavior: y_0(z) ~ -cos(z)/z for large z."""
        n = torch.tensor([0.0], dtype=torch.float64)
        z = torch.tensor([100.0, 200.0, 500.0], dtype=torch.float64)
        result = torchscience.special_functions.spherical_bessel_y(
            n.expand_as(z), z
        )
        asymptotic = -torch.cos(z) / z
        torch.testing.assert_close(result, asymptotic, rtol=1e-8, atol=1e-10)
