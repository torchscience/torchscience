...
// Backward pass for Weierstrass sigma function σ(z; g2, g3)
//
...
template <typename T>
C10_HOST_DEVICE inline T weierstrass_sigma_finite_diff_step() {
    return std::pow(std::numeric_limits<T>::epsilon(), T(1.0/3.0));
}

template <>
C10_HOST_DEVICE inline float weierstrass_sigma_finite_diff_step<float>() {
    return 1e-4f;
}

template <>
C10_HOST_DEVICE inline double weierstrass_sigma_finite_diff_step<double>() {
    return 1e-7;
}
...
template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T> weierstrass_sigma_backward(
    T gradient,
    T z,
    T g2,
    T g3
...
template <typename T>
C10_HOST_DEVICE std::tuple<c10::complex<T>, c10::complex<T>, c10::complex<T>> weierstrass_sigma_backward(
