...
def hermite_polynomial_he_linspace(
    n: int,
    ...
    hermite_polynomial_he_points instead.

    Examples
    --------
    >>> hermite_polynomial_he_linspace(5, start=-2.0, end=2.0)
    tensor([-2., -1.,  0.,  1.,  2.])

    >>> # This will raise DomainError:
    >>> hermite_polynomial_he_linspace(5)  # Missing start/end
    """
    domain = HermitePolynomialHe.DOMAIN

    if math.isinf(domain[0]) or math.isinf(domain[1]):
        if start is None or end is None:
            raise DomainError(
                f"HermitePolynomialHe has unbounded domain {domain}. "
