...
T fresnel_s_series(T z) {
  ...
  // First term: (pi/2) * z^3 / 3
  T term = pi_over_2 * z3 / static_cast<T>(3);
  T sum = term;
  T pi2_over_4 = pi_over_2 * pi_over_2;

  // Subsequent terms using recurrence relation
  for (int n = 1; n < 100; ++n) {
    // term_n / term_{n-1} = -pi^2/4 * z^4 * (4n-1) / ((2n)(2n+1) * (4n+3))
    term *= -pi2_over_4 * z4 * static_cast<T>(4 * n - 1) /
            (static_cast<T>(2 * n) * static_cast<T>(2 * n + 1) *
             static_cast<T>(4 * n + 3));
    T new_sum = sum + term;
...
c10::complex<T> fresnel_s_series_complex(c10::complex<T> z) {
  ...
  c10::complex<T> term = pi_over_2 * z3 / static_cast<T>(3);
...
void fresnel_fg_asymptotic(T z, T& f, T& g) {
  ...
  T inv_pi_z2_sq = inv_pi_z2 * inv_pi_z2;  // 1 / (pi * z^2)^2

  // Compute f(z) = 1/(pi*z) * sum_n a_n / (pi*z^2)^{2n}
  // where a_0 = 1, a_n = -a_{n-1} * (4n-3)(4n-1)
  ...
  for (int n = 1; n <= 25; ++n) {
    ...
    if (std::abs(term_f) > std::abs(sum_f) * T(0.5)) break;  // Series diverging
    T new_sum = sum_f + term_f;
    if (new_sum == sum_f) break;  // Converged
    sum_f = new_sum;
  }
  f = sum_f / (pi * z);

  // Compute g(z) = 1/(pi^2*z^3) * sum_n b_n / (pi*z^2)^{2n}
  // where b_0 = 1, b_n = -b_{n-1} * (4n-1)(4n+1)
  ...
  for (int n = 1; n <= 25; ++n) {
    ...
    if (std::abs(term_g) > std::abs(sum_g) * T(0.5)) break;  // Series diverging
    T new_sum = sum_g + term_g;
    if (new_sum == sum_g) break;  // Converged
    sum_g = new_sum;
  }
  g = sum_g / (pi * pi * z * z * z);
}
...
// Fresnel sine integral S(z) = integral from 0 to z of sin(pi*t^2/2) dt
// Properties:
//   S(0) = 0
//   S(-z) = -S(z) (odd function)
//   S(z) -> 0.5 as z -> +inf
//   S(z) -> -0.5 as z -> -inf
template <typename T>
T fresnel_s(T z) {
  ...
  // The crossover point is where both methods give comparable accuracy
  // For double precision, this is around z = 4-5
  T threshold = static_cast<T>(4.5);

  T result;
  if (az < threshold) {
    result = detail::fresnel_s_series(az);
  } else {
    T f, g;
    detail::fresnel_fg_asymptotic(az, f, g);
    T arg = pi_over_2 * az * az;
    result = static_cast<T>(0.5) - f * std::cos(arg) - g * std::sin(arg);
  }
...
c10::complex<T> fresnel_s(c10::complex<T> z) {
  ...
  if (az < threshold) {
    return detail::fresnel_s_series_complex(z);
  } else {
    c10::complex<T> f, g;
    detail::fresnel_fg_asymptotic_complex(z, f, g);
    c10::complex<T> arg = pi_over_2 * z * z;
    return c10::complex<T>(0.5, 0) - f * c10_complex_math::cos(arg) - g * c10_complex_math::sin(arg);
  }
}
