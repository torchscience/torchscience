...
class TestSphericalBesselI1:
    """Tests for the modified spherical Bessel function i_1."""
    ...
    def test_special_values(self):
        """Test special values: i_1(0) = 0, i_1(nan) = nan."""
        assert torchscience.special_functions.spherical_bessel_i_1(
            torch.tensor(0.0)
    ...
    def test_scipy_agreement_small(self):
        ...
        result = torchscience.special_functions.spherical_bessel_i_1(z)
        expected = torch.tensor(
            [scipy.special.spherical_in(1, x.item()) for x in z],
            dtype=torch.float64,
        )
    ...
    def test_closed_form(self):
        """Test i_1(z) = cosh(z)/z - sinh(z)/z^2 for non-zero z."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        result = torchscience.special_functions.spherical_bessel_i_1(z)
        expected = torch.cosh(z) / z - torch.sinh(z) / (z * z)
        torch.testing.assert_close(result, expected, rtol=1e-12, atol=1e-12)

    def test_symmetry_odd_function(self):
        """Test i_1(-z) = -i_1(z) (odd function)."""
        z_pos = torch.tensor([0.1, 1.0, 5.0, 10.0], dtype=torch.float64)
        z_neg = -z_pos
        result_pos = torchscience.special_functions.spherical_bessel_i_1(z_pos)
        result_neg = torchscience.special_functions.spherical_bessel_i_1(z_neg)
        torch.testing.assert_close(
            result_neg, -result_pos, rtol=1e-12, atol=1e-12
        )

    def test_taylor_series_near_zero(self):
        """Test Taylor series accuracy near z=0: i_1(z) = z/3 + z^3/30 + z^5/840."""
    ...
    def test_gradcheck(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.spherical_bessel_i_1, z
        )
    ...
    def test_gradient_formula(self):
        """Verify d/dz i_1(z) = i_0(z) - 2*i_1(z)/z numerically."""
        z = torch.tensor(
            [0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64, requires_grad=True
        )
        y = torchscience.special_functions.spherical_bessel_i_1(z)
        grad = torch.autograd.grad(y.sum(), z)[0]
        z_detached = z.detach()
        i0 = torchscience.special_functions.spherical_bessel_i_0(z_detached)
        i1 = torchscience.special_functions.spherical_bessel_i_1(z_detached)
        expected = i0 - 2 * i1 / z_detached
    ...
    def test_gradient_at_origin_limit(self):
        ...
        y = torchscience.special_functions.spherical_bessel_i_1(z)
        grad = torch.autograd.grad(y.sum(), z)[0]
        # d/dz i_1(z) at small z is approximately 1/3
    ...
    def test_complex_near_real_accuracy(self):
        ...
        result = torchscience.special_functions.spherical_bessel_i_1(
            z_near_real
        )
        expected = torch.tensor(
            [scipy.special.spherical_in(1, z.item()) for z in z_near_real],
            dtype=torch.complex128,
        )
        torch.testing.assert_close(result, expected, rtol=1e-10, atol=1e-10)

    def test_complex_closed_form(self):
        """Test i_1(z) = cosh(z)/z - sinh(z)/z^2 for complex z."""
        z = torch.tensor(
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real i_1."""
        x_real = torch.tensor([1.0, 2.0, 3.0, 0.5, 1.5], dtype=torch.float64)
        x_complex = x_real.to(torch.complex128)
        result_real = torchscience.special_functions.spherical_bessel_i_1(
            x_real
        )
        result_complex = torchscience.special_functions.spherical_bessel_i_1(
            x_complex
        )
        torch.testing.assert_close(
    ...
    def test_complex_odd_symmetry(self):
        """Test i_1(-z) = -i_1(z) for complex z (odd function)."""
        z = torch.tensor(
            [1.0 + 0.5j, 2.0 - 0.3j, 0.5 + 2.0j], dtype=torch.complex128
        )
        result_pos = torchscience.special_functions.spherical_bessel_i_1(z)
        result_neg = torchscience.special_functions.spherical_bessel_i_1(-z)
        torch.testing.assert_close(
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(
            torchscience.special_functions.spherical_bessel_i_1
        )(z)
        expected = torchscience.special_functions.spherical_bessel_i_1(z)
        torch.testing.assert_close(result, expected)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.spherical_bessel_i_1(z2)
    ...
    def test_broadcasting(self):
        ...
        result = torchscience.special_functions.spherical_bessel_i_1(z1 + z2)
