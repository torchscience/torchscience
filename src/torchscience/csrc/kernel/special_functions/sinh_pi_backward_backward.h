...
template <typename T>
std::tuple<T, T> sinh_pi_backward_backward(T gradient_gradient, T g, T x) {
  ...
  T g_x = gradient_gradient * g * pi * pi * sinh_pi_x;
...
std::tuple<c10::complex<T>, c10::complex<T>> sinh_pi_backward_backward(
  ...
  c10::complex<T> g_z = gradient_gradient * g * pi * pi * sinh_pi(z);
