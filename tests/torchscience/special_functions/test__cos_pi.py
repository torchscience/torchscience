...
class TestCosPi(OpTestCase):
    @property
    def descriptor(self) -> OperatorDescriptor:
        return OperatorDescriptor(
            name="cos_pi",
            func=torchscience.special_functions.cos_pi,
            arity=1,
            input_specs=[
                InputSpec(
                    name="x",
            ...
            sympy_func=lambda x: sympy.cos(sympy.pi * x),
    ...
    def test_gradcheck(self):
        ...
        torch.autograd.gradcheck(
            torchscience.special_functions.cos_pi,
            (x,),
            eps=1e-5,
            atol=1e-3,
            rtol=1e-3,
