...
template <typename T>
C10_HOST_DEVICE T regularized_gamma_p_grad_x(T a, T x) {
  if (x <= T(0)) return T(0);
...
template <typename T>
C10_HOST_DEVICE T regularized_gamma_p_grad_a(T a, T x) {
...
template <typename T>
C10_HOST_DEVICE std::tuple<T, T> regularized_gamma_p_backward(T grad_output, T a, T x) {
  return {
