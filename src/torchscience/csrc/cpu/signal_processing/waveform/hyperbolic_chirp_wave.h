...
#include "../../../kernel/signal_processing/waveform/hyperbolic_chirp_wave.h"

namespace torchscience::cpu::signal_processing::waveform {

inline at::Tensor hyperbolic_chirp_wave(
  ...
  TORCH_CHECK(
      ...
      "hyperbolic_chirp_wave: Exactly one of n or t must be provided");

  if (n.has_value()) {
    TORCH_CHECK(n.value() >= 0, "hyperbolic_chirp_wave: n must be non-negative, got ", n.value());
  }

  TORCH_CHECK(t1 > 0, "hyperbolic_chirp_wave: t1 must be positive, got ", t1);
  ...
  if (t.has_value()) {
    ...
    TORCH_CHECK(sample_rate > 0, "hyperbolic_chirp_wave: sample_rate must be positive");
    auto options = at::TensorOptions()
        .dtype(dtype.value_or(at::kFloat))
        .device(device.value_or(at::kCPU));
  ...
  AT_DISPATCH_FLOATING_TYPES(output.scalar_type(), "hyperbolic_chirp_wave_cpu", [&] {
    ...
    at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
      for (int64_t b = begin; b < end; ++b) {
        ...
        for (int64_t i = 0; i < n_samples; ++i) {
          out_data[b * n_samples + i] = kernel::hyperbolic_chirp_wave_kernel(
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
  m.impl("hyperbolic_chirp_wave", &torchscience::cpu::signal_processing::waveform::hyperbolic_chirp_wave);
}
