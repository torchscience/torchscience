...
class TestStruveH0:
    """Tests for the Struve function H_0."""
    ...
    def test_special_values(self):
        """Test special values: H_0(0) = 0, H_0(nan) = nan."""
        # H_0(0) = 0
        assert torchscience.special_functions.struve_h_0(
            torch.tensor(0.0)
    ...
    def test_scipy_agreement_small(self):
        ...
        result = torchscience.special_functions.struve_h_0(z)
        expected = torch.tensor(
            [scipy.special.struve(0, x.item()) for x in z], dtype=torch.float64
        )
    ...
    def test_symmetry_odd_function(self):
        """H_0(-z) = -H_0(z) (odd function)."""
        z = torch.tensor(
            [0.1, 0.5, 1.0, 2.0, 4.0, 4.9, 5.1, 15.0, 20.0],
            dtype=torch.float64,
        )
        result_pos = torchscience.special_functions.struve_h_0(z)
        result_neg = torchscience.special_functions.struve_h_0(-z)
        torch.testing.assert_close(
            result_pos, -result_neg, rtol=1e-10, atol=1e-10
        )

    def test_negative_real(self):
        """Test H_0 for negative real arguments (odd function)."""
        z_pos = torch.tensor([0.1, 1.0, 5.0, 10.0, 20.0], dtype=torch.float64)
        z_neg = -z_pos
        result_pos = torchscience.special_functions.struve_h_0(z_pos)
        result_neg = torchscience.special_functions.struve_h_0(z_neg)
        torch.testing.assert_close(
            result_pos, -result_neg, rtol=1e-10, atol=1e-10
        )
    ...
    def test_specific_values(self):
        ...
        test_cases = [
            (0.5, scipy.special.struve(0, 0.5)),
            (1.0, scipy.special.struve(0, 1.0)),
            (2.0, scipy.special.struve(0, 2.0)),
            (5.0, scipy.special.struve(0, 5.0)),
            (10.0, scipy.special.struve(0, 10.0)),
        ]
    ...
    def test_gradcheck(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.struve_h_0, z, eps=1e-6
        )
    ...
    def test_gradient_identity(self):
        """Verify d/dz H_0(z) = 2/pi - H_1(z) numerically."""
        z = torch.tensor(
            [0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64, requires_grad=True
        )
        y = torchscience.special_functions.struve_h_0(z)
        grad = torch.autograd.grad(y.sum(), z)[0]
        # Expected: d/dz H_0(z) = 2/pi - H_1(z)
        two_over_pi = 2.0 / math.pi
        expected = two_over_pi - torchscience.special_functions.struve_h_1(
            z.detach()
        )
    ...
    def test_complex_mpmath_agreement(self):
        ...
        expected_values = [complex(mpmath.struveh(0, zv)) for zv in z_values]
    ...
    def test_complex_near_real_accuracy(self):
        ...
        result = torchscience.special_functions.struve_h_0(z_near_real)
        expected = torch.tensor(
            [complex(mpmath.struveh(0, z.item())) for z in z_near_real],
    ...
    def test_complex_farther_from_real(self):
        ...
        result = torchscience.special_functions.struve_h_0(z_far)
        expected = torch.tensor(
            [complex(mpmath.struveh(0, z.item())) for z in z_far],
            dtype=torch.complex128,
        )
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real H_0."""
        x_real = torch.tensor([1.0, 2.0, 3.0, 0.5, 1.5], dtype=torch.float64)
        x_complex = x_real.to(torch.complex128)
        result_real = torchscience.special_functions.struve_h_0(x_real)
        result_complex = torchscience.special_functions.struve_h_0(x_complex)
        torch.testing.assert_close(
    ...
    def test_complex_odd_symmetry(self):
        """Test H_0(-z) = -H_0(z) for complex arguments."""
        z = torch.tensor(
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.struve_h_0(z)
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(torchscience.special_functions.struve_h_0)(z)
        expected = torchscience.special_functions.struve_h_0(z)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.struve_h_0)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.struve_h_0(z2)
    ...
    def test_broadcasting(self):
        ...
        result = torchscience.special_functions.struve_h_0(z1 + z2)
