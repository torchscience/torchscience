...
#include "../../../kernel/signal_processing/waveform/impulse_wave.h"

namespace torchscience::cpu::signal_processing::waveform {

inline at::Tensor impulse_wave(
    int64_t n,
  ...
  TORCH_CHECK(n >= 0, "impulse_wave: n must be non-negative, got ", n);
  ...
  AT_DISPATCH_FLOATING_TYPES(output.scalar_type(), "impulse_wave_cpu", [&] {
    ...
    at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
      for (int64_t b = begin; b < end; ++b) {
        ...
        for (int64_t i = 0; i < n; ++i) {
          out_data[b * n + i] = kernel::impulse_wave_kernel<scalar_t>(i, pos, amp);
        }
      }
    });
  });
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
  m.impl("impulse_wave", &torchscience::cpu::signal_processing::waveform::impulse_wave);
}
