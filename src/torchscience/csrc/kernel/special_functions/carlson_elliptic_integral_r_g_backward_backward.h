...
#include "carlson_elliptic_integral_r_g_backward.h"

namespace torchscience::kernel::special_functions {

template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T, T> carlson_elliptic_integral_r_g_backward_backward(
    T gg_x,
    T gg_y,
    T gg_z,
    T gradient,
    ...
    auto [dx, dy, dz] = carlson_elliptic_integral_r_g_backward(T(1), x, y, z);
    ...
    auto [dx_px, dy_px, dz_px] = carlson_elliptic_integral_r_g_backward(T(1), x + eps, y, z);
    auto [dx_py, dy_py, dz_py] = carlson_elliptic_integral_r_g_backward(T(1), x, y + eps, z);
    auto [dx_pz, dy_pz, dz_pz] = carlson_elliptic_integral_r_g_backward(T(1), x, y, z + eps);
...
carlson_elliptic_integral_r_g_backward_backward(
    c10::complex<T> gg_x,
    c10::complex<T> gg_y,
    c10::complex<T> gg_z,
    ...
    auto [dx, dy, dz] = carlson_elliptic_integral_r_g_backward(one, x, y, z);

    c10::complex<T> grad_gradient = gg_x * dx + gg_y * dy + gg_z * dz;

    auto [dx_px, dy_px, dz_px] = carlson_elliptic_integral_r_g_backward(one, x + eps_c, y, z);
    auto [dx_py, dy_py, dz_py] = carlson_elliptic_integral_r_g_backward(one, x, y + eps_c, z);
    auto [dx_pz, dy_pz, dz_pz] = carlson_elliptic_integral_r_g_backward(one, x, y, z + eps_c);
