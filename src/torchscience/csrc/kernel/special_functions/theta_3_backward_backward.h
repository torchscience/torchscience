...
template <typename T>
C10_HOST_DEVICE inline T theta3_finite_diff_step2() {
    return std::pow(std::numeric_limits<T>::epsilon(), T(0.25));
}

template <>
C10_HOST_DEVICE inline float theta3_finite_diff_step2<float>() {
    return 1e-2f;
}

template <>
C10_HOST_DEVICE inline double theta3_finite_diff_step2<double>() {
    return 1e-4;
}
...
template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T> theta_3_backward_backward(
    T gg_z,
    T gg_q,
    T gradient,
    T z,
