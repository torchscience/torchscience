...
T anger_j_derivative_n(T n, T z) {
    ...
    for (int j = 0; j < 10; ++j) {
        ...
        T integrand_pos = -theta_pos * std::sin(arg_pos);
        ...
        T integrand_neg = -theta_neg * std::sin(arg_neg);
...
c10::complex<T> anger_j_derivative_n(c10::complex<T> n, c10::complex<T> z) {
    ...
    for (int j = 0; j < 10; ++j) {
        ...
        c10::complex<T> integrand_pos = -theta_pos * c10_complex_math::sin(arg_pos);
        result += weights[j] * integrand_pos;

        T t_neg = -nodes[j];
        T theta_neg = half_pi * (t_neg + T(1));
        c10::complex<T> arg_neg = n * theta_neg - z * std::sin(theta_neg);
        c10::complex<T> integrand_neg = -theta_neg * c10_complex_math::sin(arg_neg);
        result += weights[j] * integrand_neg;
    }
...
// Backward pass for Anger function
// Returns (grad_n, grad_z)
//
// Derivative of Anger function w.r.t. z:
// d/dz J_nu(z) = (1/2) * [J_{nu-1}(z) - J_{nu+1}(z)]
//
...
std::tuple<T, T> anger_j_backward(T grad_output, T n, T z) {
    ...
    T grad_z = grad_output * T(0.5) * (j_nu_minus_1 - j_nu_plus_1);
...
std::tuple<c10::complex<T>, c10::complex<T>> anger_j_backward(
    ...
    c10::complex<T> grad_n = grad_output * anger_j_derivative_n(n, z);

    c10::complex<T> j_nu_minus_1 = anger_j(n - c10::complex<T>(T(1), T(0)), z);
    c10::complex<T> j_nu_plus_1 = anger_j(n + c10::complex<T>(T(1), T(0)), z);
    c10::complex<T> grad_z = grad_output * c10::complex<T>(T(0.5), T(0)) * (j_nu_minus_1 - j_nu_plus_1);
