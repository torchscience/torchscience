...
class TestModifiedBesselK1:
    """Tests for the modified Bessel function of the second kind K_1."""
    ...
    def test_special_values(self):
        """Test special values: K_1(0) = +inf, K_1(inf) = 0, K_1(nan) = nan."""
        # K_1(0) = +infinity (singularity)
        assert torchscience.special_functions.modified_bessel_k_1(
            torch.tensor(0.0)
        ).item() == float("inf")
    ...
    def test_scipy_agreement_small(self):
        ...
        result = torchscience.special_functions.modified_bessel_k_1(z)
        expected = torch.tensor(
            [scipy.special.k1(x.item()) for x in z], dtype=torch.float64
        )
    ...
    def test_negative_real_returns_nan(self):
        """K_1(z) for negative real z should return NaN (not defined)."""
        z = torch.tensor([-0.1, -1.0, -5.0, -10.0], dtype=torch.float64)
        result = torchscience.special_functions.modified_bessel_k_1(z)
        assert result.isnan().all()

    def test_near_zero_singularity(self):
        """Test behavior near z=0 singularity: K_1(z) ~ 1/z for small z."""
    ...
    def test_exponential_decay(self):
        """Test exponential decay at large z: K_1(z) ~ sqrt(pi/2z) * exp(-z)."""
    ...
    def test_gradcheck(self):
        ...
        # Avoid z=0 where K_1 has a singularity
        z = (
            torch.rand(10, dtype=torch.float64) * 4 + 0.5
        )  # values in [0.5, 4.5]
        z = z.clone().requires_grad_(True)
        assert torch.autograd.gradcheck(
            torchscience.special_functions.modified_bessel_k_1, z
        )
    ...
    def test_gradient_identity(self):
        """Verify d/dz K_1(z) = -K_0(z) - K_1(z)/z numerically."""
        z = torch.tensor(
            [0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64, requires_grad=True
        )
        y = torchscience.special_functions.modified_bessel_k_1(z)
        grad = torch.autograd.grad(y.sum(), z)[0]
        z_detach = z.detach()
        expected = (
            -torchscience.special_functions.modified_bessel_k_0(z_detach)
            - torchscience.special_functions.modified_bessel_k_1(z_detach)
            / z_detach
        )
    ...
    def test_complex_near_real_accuracy(self):
        ...
        result = torchscience.special_functions.modified_bessel_k_1(
            z_near_real
        )
        expected = torch.tensor(
            [scipy.special.kv(1, z.item()) for z in z_near_real],
    ...
    def test_complex_farther_from_real(self):
        ...
        result = torchscience.special_functions.modified_bessel_k_1(z_far)
        expected = torch.tensor(
            [scipy.special.kv(1, z.item()) for z in z_far],
            dtype=torch.complex128,
        )
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on positive real axis match real K_1."""
        x_real = torch.tensor([0.5, 1.0, 2.0, 3.0, 5.0], dtype=torch.float64)
        x_complex = x_real.to(torch.complex128)
        result_real = torchscience.special_functions.modified_bessel_k_1(
            x_real
        )
        result_complex = torchscience.special_functions.modified_bessel_k_1(
            x_complex
        )
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(
            torchscience.special_functions.modified_bessel_k_1
        )(z)
        expected = torchscience.special_functions.modified_bessel_k_1(z)
        torch.testing.assert_close(result, expected)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.modified_bessel_k_1(z2)
    ...
    def test_broadcasting(self):
        ...
        result = torchscience.special_functions.modified_bessel_k_1(z1 + z2)
