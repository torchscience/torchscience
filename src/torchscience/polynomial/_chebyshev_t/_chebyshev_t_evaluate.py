...
from torchscience.polynomial._chebyshev_t._chebyshev_t import ChebyshevT


def chebyshev_t_evaluate(c: ChebyshevT, x: Tensor) -> Tensor:
    ...
    Examples
    --------
    >>> c = chebyshev_t(torch.tensor([1.0, 2.0, 3.0]))  # 1 + 2*T_1 + 3*T_2
    >>> chebyshev_t_evaluate(c, torch.tensor([0.0]))
    tensor([-2.])  # 1 + 0 + 3*(-1) = -2
    """
    coeffs = c.coeffs
    n = coeffs.shape[-1]
