...
#include "whittaker_m_backward.h"

namespace torchscience::kernel::special_functions {

// Second-order backward pass for Whittaker M function
...
template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T, T> whittaker_m_backward_backward(
  T gg_kappa,
  T gg_mu,
  T gg_z,
  T grad_output,
  ...
  auto [dkappa_kappa_plus, dmu_kappa_plus, dz_kappa_plus] =
    whittaker_m_backward(T(1), kappa + eps, mu, z);
  ...
  auto [dkappa_mu_plus, dmu_mu_plus, dz_mu_plus] =
    whittaker_m_backward(T(1), kappa, mu + eps, z);
  ...
  auto [dkappa_z_plus, dmu_z_plus, dz_z_plus] =
    whittaker_m_backward(T(1), kappa, mu, z + eps);
