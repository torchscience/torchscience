...
template <typename T>
C10_HOST_DEVICE inline T bessel_zero_tolerance() {
  return T(1e-12);  // Default for low-precision types
}

template <>
C10_HOST_DEVICE inline float bessel_zero_tolerance<float>() { return 1e-6f; }

template <>
C10_HOST_DEVICE inline double bessel_zero_tolerance<double>() { return 1e-12; }

} // namespace detail

// Real backward: d/dz I₁(z) = I₀(z) - I₁(z)/z
template <typename T>
C10_HOST_DEVICE T modified_bessel_i_1_backward(T grad_output, T z) {
...
// Complex backward
template <typename T>
C10_HOST_DEVICE c10::complex<T> modified_bessel_i_1_backward(c10::complex<T> grad_output, c10::complex<T> z) {
