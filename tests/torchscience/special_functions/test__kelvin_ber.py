...
class TestKelvinBer:
    """Tests for the Kelvin function ber(x)."""
    ...
    def test_special_values(self):
        """Test special values: ber(0) = 1, ber(nan) = nan."""
        assert torchscience.special_functions.kelvin_ber(
            torch.tensor(0.0)
    ...
    def test_scipy_agreement_small(self):
        ...
        result = torchscience.special_functions.kelvin_ber(x)
        expected = torch.tensor(
            [scipy.special.ber(val.item()) for val in x], dtype=torch.float64
        )
    ...
    def test_symmetry_even_function(self):
        """ber(-x) = ber(x) (even function)."""
        x = torch.tensor(
            [0.1, 0.5, 1.0, 2.0, 4.0, 5.0, 7.0],
            dtype=torch.float64,
        )
        result_pos = torchscience.special_functions.kelvin_ber(x)
        result_neg = torchscience.special_functions.kelvin_ber(-x)
    ...
    def test_known_values(self):
        ...
        # Reference values from scipy.special.ber
        test_cases = [
            (0.0, 1.0),
            (1.0, 0.984381781213087),
            (2.0, 0.751734182713808),
            (5.0, -6.230082478666358),
        ]
    ...
    def test_gradcheck(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.kelvin_ber, x, rtol=1e-4, atol=1e-4
        )
    ...
    def test_gradgradcheck(self):
        ...
        assert torch.autograd.gradgradcheck(
            torchscience.special_functions.kelvin_ber, x, rtol=1e-3, atol=1e-3
        )

    def test_gradient_at_zero(self):
        """Verify ber'(0) = 0."""
        x = torch.tensor([0.0], dtype=torch.float64, requires_grad=True)
        y = torchscience.special_functions.kelvin_ber(x)
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real ber."""
        x_real = torch.tensor([0.5, 1.0, 2.0, 3.0], dtype=torch.float64)
        x_complex = x_real.to(torch.complex128)
        result_real = torchscience.special_functions.kelvin_ber(x_real)
        result_complex = torchscience.special_functions.kelvin_ber(x_complex)
        torch.testing.assert_close(
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.kelvin_ber(x)
    ...
    def test_vmap(self):
        ...
        result = torch.vmap(torchscience.special_functions.kelvin_ber)(x)
        expected = torchscience.special_functions.kelvin_ber(x)
        torch.testing.assert_close(result, expected)

    def test_compile(self):
        """Verify torch.compile compatibility."""
        compiled_fn = torch.compile(torchscience.special_functions.kelvin_ber)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.kelvin_ber(x2)
    ...
    def test_broadcasting(self):
        ...
        result = torchscience.special_functions.kelvin_ber(x1 + x2)
