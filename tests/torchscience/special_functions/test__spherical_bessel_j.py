...
class TestSphericalBesselJ:
    """Tests for the spherical Bessel function j_n(z) of general order n."""
    ...
    def test_order_zero_matches_j0(self):
        """Test j_0(z) via spherical_bessel_j(0, z) matches spherical_bessel_j_0(z)."""
        z = torch.tensor([0.0, 0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        n = torch.zeros_like(z)
        result = torchscience.special_functions.spherical_bessel_j(n, z)
        expected = torchscience.special_functions.spherical_bessel_j_0(z)
        torch.testing.assert_close(result, expected, rtol=1e-10, atol=1e-10)

    def test_order_one_matches_j1(self):
        """Test j_1(z) via spherical_bessel_j(1, z) matches spherical_bessel_j_1(z)."""
    ...
    def test_scipy_agreement_integer_orders(self):
        """Test agreement with scipy.special.spherical_jn for integer orders."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        for order in [0, 1, 2, 3, 5, 10]:
            n = torch.full_like(z, float(order))
            result = torchscience.special_functions.spherical_bessel_j(n, z)
            expected = torch.tensor(
                [scipy.special.spherical_jn(order, x.item()) for x in z],
                dtype=torch.float64,
            )
    ...
    def test_scipy_agreement_half_order(self):
        ...
        for order in [0.5, 1.5, 2.5]:
            ...
            result = torchscience.special_functions.spherical_bessel_j(n, z)
            # For half-integer order, use Bessel J relation
            nu = order + 0.5
            expected = torch.tensor(
                [
                    math.sqrt(math.pi / (2 * x.item()))
                    * scipy.special.jv(nu, x.item())
                    for x in z
                ],
                dtype=torch.float64,
            )
    ...
    def test_nan_input(self):
        """Test j_n(NaN) = NaN and j_NaN(z) = NaN."""
    ...
    def test_recurrence_relation(self):
        """Test recurrence: j_{n-1}(z) + j_{n+1}(z) = (2n+1)/z * j_n(z)."""
        ...
        for order in [1.0, 2.0, 3.0, 0.5, 1.5]:
            ...
            j_n = torchscience.special_functions.spherical_bessel_j(n, z)
            j_nm1 = torchscience.special_functions.spherical_bessel_j(
                n_minus, z
            )
            j_np1 = torchscience.special_functions.spherical_bessel_j(
                n_plus, z
            )
    ...
    def test_sinc_identity_n0(self):
        """Test j_0(z) = sin(z)/z for non-zero z."""
        z = torch.tensor([0.5, 1.0, 2.0, 5.0, 10.0], dtype=torch.float64)
        n = torch.zeros_like(z)
        result = torchscience.special_functions.spherical_bessel_j(n, z)
        expected = torch.sin(z) / z
    ...
    def test_gradcheck_z_integer_order(self):
        ...
        def func(z_):
            return torchscience.special_functions.spherical_bessel_j(n, z_)
    ...
    def test_gradcheck_both_inputs(self):
        ...
        def func(n_, z_):
            return torchscience.special_functions.spherical_bessel_j(n_, z_)
    ...
    def test_gradient_z_identity(self):
        """Verify d/dz j_n(z) = (n/z)*j_n(z) - j_{n+1}(z)."""
        n = torch.tensor([1.0, 2.0, 0.5, 1.5], dtype=torch.float64)
        z = torch.tensor(
            [1.0, 2.0, 3.0, 4.0], dtype=torch.float64, requires_grad=True
        )

        y = torchscience.special_functions.spherical_bessel_j(n, z)
        grad = torch.autograd.grad(y.sum(), z)[0]

        j_n = torchscience.special_functions.spherical_bessel_j(n, z.detach())
        j_np1 = torchscience.special_functions.spherical_bessel_j(
            n + 1, z.detach()
        )
        expected = (n / z.detach()) * j_n - j_np1
    ...
    def test_complex_near_real_accuracy(self):
        ...
        expected = torch.tensor(
            [
                scipy.special.spherical_jn(int(n[i].real.item()), z[i].item())
    ...
    def test_complex_on_real_axis_matches_real(self):
        """Test complex numbers on real axis match real j_n."""
        ...
        result_real = torchscience.special_functions.spherical_bessel_j(
            n_real, z_real
        )
        result_complex = torchscience.special_functions.spherical_bessel_j(
            n_complex, z_complex
        )
    ...
    def test_autocast(self):
        ...
        expected = torchscience.special_functions.spherical_bessel_j(n, z)
    ...
    def test_vmap(self):
        ...
        def fn(z_row):
            return torchscience.special_functions.spherical_bessel_j(n, z_row)

        result = torch.vmap(fn)(z)
        expected = torchscience.special_functions.spherical_bessel_j(
            n.unsqueeze(0), z
        )
        torch.testing.assert_close(result, expected)
    ...
    def test_compile(self):
        ...
        compiled_fn = torch.compile(
            torchscience.special_functions.spherical_bessel_j
        )
        n = torch.tensor([0.0, 1.0, 2.0], dtype=torch.float64)
    ...
    def test_compile_with_autograd(self):
        ...
        expected = torchscience.special_functions.spherical_bessel_j(n, z2)
    ...
    def test_large_z(self):
        ...
        result = torchscience.special_functions.spherical_bessel_j(
            n.unsqueeze(1), z
        )
        expected = torch.tensor(
            [
                [
                    scipy.special.spherical_jn(int(ni.item()), zi.item())
                    for zi in z
                ]
                for ni in n
            ],
    ...
    def test_zeros_at_expected_locations(self):
        """Test that j_0 has zeros at multiples of pi."""
        n = torch.tensor([0.0], dtype=torch.float64)
        n_values = torch.tensor([1, 2, 3, 4, 5], dtype=torch.float64)
        z = n_values * math.pi
        result = torchscience.special_functions.spherical_bessel_j(
            n.expand_as(z), z
        )
