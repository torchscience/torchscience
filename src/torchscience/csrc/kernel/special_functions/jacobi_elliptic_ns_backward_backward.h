...
template <typename T>
C10_HOST_DEVICE T compute_d2ns_du2(T u, T m) {
...
template <typename T>
C10_HOST_DEVICE T compute_d2ns_dm2(T u, T m) {
    const T h = std::pow(std::numeric_limits<T>::epsilon(), T(1.0/3.0));
...
template <typename T>
C10_HOST_DEVICE T compute_d2ns_dudm(T u, T m) {
...
template <typename T>
C10_HOST_DEVICE c10::complex<T> compute_d2ns_du2(c10::complex<T> u, c10::complex<T> m) {
    const T h = std::pow(std::numeric_limits<T>::epsilon(), T(1.0/3.0));
...
template <typename T>
C10_HOST_DEVICE c10::complex<T> compute_d2ns_dm2(c10::complex<T> u, c10::complex<T> m) {
    const T h = std::pow(std::numeric_limits<T>::epsilon(), T(1.0/3.0));
    c10::complex<T> h_c(h, T(0));
    c10::complex<T> two(T(2), T(0));
...
template <typename T>
C10_HOST_DEVICE c10::complex<T> compute_d2ns_dudm(c10::complex<T> u, c10::complex<T> m) {
    const T h = std::pow(std::numeric_limits<T>::epsilon(), T(1.0/4.0));
    c10::complex<T> h_c(h, T(0));
...
// Compute dns/du
template <typename T>
C10_HOST_DEVICE T compute_dns_du(T u, T m) {
...
// Compute dns/dm
template <typename T>
C10_HOST_DEVICE T compute_dns_dm(T u, T m) {
    auto [grad_u, grad_m] = jacobi_elliptic_ns_backward(T(1), u, m);
    return grad_m;
}

template <typename T>
C10_HOST_DEVICE c10::complex<T> compute_dns_du(c10::complex<T> u, c10::complex<T> m) {
...
template <typename T>
C10_HOST_DEVICE c10::complex<T> compute_dns_dm(c10::complex<T> u, c10::complex<T> m) {
    c10::complex<T> one(T(1), T(0));
    auto [grad_u, grad_m] = jacobi_elliptic_ns_backward(one, u, m);
    return grad_m;
}
...
template <typename T>
C10_HOST_DEVICE std::tuple<T, T, T> jacobi_elliptic_ns_backward_backward(
    T gg_u,
    T gg_m,
    T grad,
    T u,
...
template <typename T>
C10_HOST_DEVICE std::tuple<c10::complex<T>, c10::complex<T>, c10::complex<T>> jacobi_elliptic_ns_backward_backward(
    c10::complex<T> gg_u,
    c10::complex<T> gg_m,
    c10::complex<T> grad,
    c10::complex<T> u,
