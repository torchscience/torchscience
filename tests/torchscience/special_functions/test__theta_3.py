...
class TestTheta3:
    """Tests for Jacobi theta function theta_3(z, q)."""

    def test_forward_zero_nome(self):
        """theta_3(z, 0) = 1."""
        z = torch.tensor([1.0], dtype=torch.float64)
        q = torch.tensor([0.0], dtype=torch.float64)
        result = torchscience.special_functions.theta_3(z, q)
        expected = torch.tensor([1.0], dtype=torch.float64)
        torch.testing.assert_close(result, expected, rtol=1e-10, atol=1e-10)

    def test_forward_even_symmetry(self):
        """theta_3(-z, q) = theta_3(z, q) (even function)."""
        z = torch.tensor([0.5], dtype=torch.float64)
        q = torch.tensor([0.3], dtype=torch.float64)
        result_pos = torchscience.special_functions.theta_3(z, q)
        result_neg = torchscience.special_functions.theta_3(-z, q)
    ...
    def test_forward_against_mpmath(self):
        ...
        for z_val, q_val in test_cases:
            ...
            result = torchscience.special_functions.theta_3(z, q)
            expected = mpmath_jtheta(3, z_val, q_val)
            torch.testing.assert_close(
                result,
    ...
    def test_gradient(self):
        ...
        assert torch.autograd.gradcheck(
            torchscience.special_functions.theta_3,
            (z, q),
            eps=1e-5,
            atol=1e-3,
            rtol=1e-3,
