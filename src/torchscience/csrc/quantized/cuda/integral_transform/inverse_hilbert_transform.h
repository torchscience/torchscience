...
/**
 * Quantized CUDA implementation of inverse Hilbert transform.
 */
inline at::Tensor inverse_hilbert_transform(
    ...
    TORCH_CHECK(
        ...
        "inverse_hilbert_transform (QuantizedCUDA) expects quantized CUDA tensor"
    );
...
/**
 * Backward pass for quantized CUDA inverse Hilbert transform.
 */
inline at::Tensor inverse_hilbert_transform_backward(
    ...
    TORCH_CHECK(
        input.is_quantized() && input.is_cuda(),
        "inverse_hilbert_transform_backward (QuantizedCUDA) expects quantized CUDA tensor for input"
    );
...
/**
 * Double backward pass for quantized CUDA inverse Hilbert transform.
 */
...
TORCH_LIBRARY_IMPL(torchscience, QuantizedCUDA, module) {
    module.impl(
        ...
        &torchscience::quantized::cuda::integral_transform::inverse_hilbert_transform
    );
    module.impl(
        "inverse_hilbert_transform_backward",
        &torchscience::quantized::cuda::integral_transform::inverse_hilbert_transform_backward
    );
    module.impl(
        "inverse_hilbert_transform_backward_backward",
        &torchscience::quantized::cuda::integral_transform::inverse_hilbert_transform_backward_backward
    );
}
