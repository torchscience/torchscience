...
#include "torchscience/csrc/kernel/information_theory/kullback_leibler_divergence_backward.h"
 ...
 * @param dim The dimension along which to compute divergence
 * @param input_type One of "probability", "log_probability", or "logits"
 * @return Tensor of probabilities
 */
inline at::Tensor js_preprocess_input(
    ...
    if (input_type == "probability") {
        ...
        TORCH_CHECK(
            false,
            "jensen_shannon_divergence: input_type must be 'probability', 'log_probability', or 'logits', got '",
            input_type, "'"
        );
    }
}
 ...
 * @param reduction One of "none", "mean", "batchmean", or "sum"
 * @param batch_size The batch size for batchmean reduction
 * @return Reduced tensor
 */
inline at::Tensor js_apply_reduction(
    ...
    if (reduction == "none") {
        ...
        TORCH_CHECK(
            false,
            "jensen_shannon_divergence: reduction must be 'none', 'mean', 'batchmean', or 'sum', got '",
            reduction, "'"
        );
    }
}
 ...
 *
 * @param base Optional log base (nullopt for natural log)
 * @return Scale factor to convert from natural log to specified base
 */
template <typename T>
inline T get_log_base_scale(const c10::optional<double>& base) {
    if (base.has_value()) {
        return static_cast<T>(1.0 / std::log(base.value()));
    }
    return T(1);
}
...
inline at::Tensor jensen_shannon_divergence(
    ...
    at::Tensor p_prob = js_preprocess_input(p, dim, input_type);
    at::Tensor q_prob = js_preprocess_input(q, dim, input_type);

    if (pairwise) {
        // Pairwise mode: compute D_JS(p_i || q_j) for all pairs
        TORCH_CHECK(
            p_prob.dim() == 2 && q_prob.dim() == 2,
            "jensen_shannon_divergence: pairwise mode requires 2D tensors, got ",
            p_prob.dim(), "D and ", q_prob.dim(), "D"
        );
        TORCH_CHECK(
            p_prob.size(1) == q_prob.size(1),
            "jensen_shannon_divergence: feature dimensions must match, got ",
        ...
        AT_DISPATCH_FLOATING_TYPES_AND2(
            ...
            [&]() {
                ...
                scalar_t log_base_scale = get_log_base_scale<scalar_t>(base);

                at::parallel_for(0, m * n, 0, [&](int64_t begin, int64_t end) {
                    for (int64_t idx = begin; idx < end; ++idx) {
                        int64_t i = idx / n;
                        int64_t j = idx % n;
                        out_ptr[idx] = torchscience::kernel::information_theory::js_divergence_kernel<scalar_t>(
                            p_ptr + i * d,
                            q_ptr + j * d,
                            d,
                            log_base_scale
        ...
        return js_apply_reduction(output, reduction, m);
    } else {
        // Standard mode: element-wise divergence
        TORCH_CHECK(
            p_prob.sizes() == q_prob.sizes(),
            "jensen_shannon_divergence: tensor shapes must match, got ",
        ...
        TORCH_CHECK(
            dim >= 0 && dim < ndim,
            "jensen_shannon_divergence: dim out of range, got ", dim
        );
        ...
        return js_apply_reduction(output, reduction, batch_size);
    }
}
...
inline std::tuple<at::Tensor, at::Tensor> jensen_shannon_divergence_backward(
    ...
    if (pairwise) {
        ...
        if (reduction == "mean") {
            ...
            scale = 1.0 / static_cast<double>(m);
        } else if (reduction == "sum") {
            scale = 1.0;
        }
        ...
        AT_DISPATCH_FLOATING_TYPES_AND2(
            at::kBFloat16, at::kHalf,
            p_prob.scalar_type(),
            "jensen_shannon_divergence_backward_pairwise_cpu",
            [&]() {
                const scalar_t* p_ptr = p_contig.data_ptr<scalar_t>();
                const scalar_t* q_ptr = q_contig.data_ptr<scalar_t>();
                ...
                for (int64_t i = 0; i < m; ++i) {
                    for (int64_t j = 0; j < n; ++j) {
                        ...
                        torchscience::kernel::information_theory::js_divergence_backward_kernel<scalar_t>(
                            grad_val,
                            p_ptr + i * d,
                            q_ptr + j * d,
                            d,
 ...
 * @param base Optional log base
 * @param pairwise Whether pairwise mode was used
 * @return Tuple of gradients (grad_grad_output, grad_p, grad_q)
 */
inline std::tuple<at::Tensor, at::Tensor, at::Tensor> jensen_shannon_divergence_backward_backward(
    ...
    if (pairwise) {
        ...
        at::Tensor p_contig = p_prob.contiguous();
        at::Tensor q_contig = q_prob.contiguous();
        at::Tensor gg_p_contig = gg_p.defined() ? gg_p.contiguous() : at::Tensor();
        at::Tensor gg_q_contig = gg_q.defined() ? gg_q.contiguous() : at::Tensor();
        at::Tensor grad_out_contig = grad_output.contiguous();
        ...
        // Output shape for grad_grad_output matches grad_output
        if (reduction == "none") {
            grad_grad_output = at::zeros({m, n}, p_prob.options());
        } else {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND2(
            at::kBFloat16, at::kHalf,
            p_prob.scalar_type(),
            "jensen_shannon_divergence_backward_backward_pairwise_cpu",
            [&]() {
                const scalar_t* p_ptr = p_contig.data_ptr<scalar_t>();
                const scalar_t* q_ptr = q_contig.data_ptr<scalar_t>();
                const scalar_t* gg_p_ptr = gg_p_contig.defined() ? gg_p_contig.data_ptr<scalar_t>() : nullptr;
                const scalar_t* gg_q_ptr = gg_q_contig.defined() ? gg_q_contig.data_ptr<scalar_t>() : nullptr;
                ...
                for (int64_t i = 0; i < m; ++i) {
                    for (int64_t j = 0; j < n; ++j) {
                        scalar_t grad_val;
                        if (reduction == "none") {
                            grad_val = grad_out_ptr[i * n + j];
                        } else {
                            grad_val = grad_out_ptr[0] * scale_t;
                        }

                        scalar_t grad_grad_out_val = scalar_t(0);
                        torchscience::kernel::information_theory::js_divergence_backward_backward_kernel<scalar_t>(
                            gg_p_ptr ? gg_p_ptr + i * d : nullptr,
                            gg_q_ptr ? gg_q_ptr + j * d : nullptr,
                            grad_val,
                            p_ptr + i * d,
                            q_ptr + j * d,
                            d,
                        ...
                        if (reduction == "none") {
                            grad_grad_out_ptr[i * n + j] = grad_grad_out_val;
                        } else {
        ...
        // Compute reduction scale factor
        double scale = 1.0;
        if (reduction == "mean" || reduction == "batchmean") {
            scale = 1.0 / static_cast<double>(batch_size);
        }

        at::Tensor grad_out_flat = grad_output.contiguous().view({-1});

        // Output shape for grad_grad_output
        if (reduction == "none") {
        ...
        // Reshape grad_grad_output if needed
        if (reduction == "none") {
            std::vector<int64_t> output_shape;
            for (int64_t i = 0; i < ndim; ++i) {
                if (i != dim) {
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
    m.impl("jensen_shannon_divergence", &torchscience::cpu::information_theory::jensen_shannon_divergence);
    m.impl("jensen_shannon_divergence_backward", &torchscience::cpu::information_theory::jensen_shannon_divergence_backward);
    m.impl("jensen_shannon_divergence_backward_backward", &torchscience::cpu::information_theory::jensen_shannon_divergence_backward_backward);
}
