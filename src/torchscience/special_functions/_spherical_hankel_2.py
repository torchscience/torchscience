...
def spherical_hankel_2(n: Tensor, z: Tensor) -> Tensor:
    r"""
    Spherical Hankel function of the second kind of general order n.

    Computes the spherical Hankel function of the second kind h_n^(2)(z) evaluated
    at each element of the input tensors, where n is the order and z is the argument.

    Mathematical Definition
    -----------------------
    The spherical Hankel function of the second kind of order n is defined as:

    .. math::

       h_n^{(2)}(z) = j_n(z) - i \cdot y_n(z)
    ...
    .. math::
       ...
       h_n^{(2)}(z) = \sqrt{\frac{\pi}{2z}} H_{n+1/2}^{(2)}(z)

    Relation to the first kind:

    .. math::

       h_n^{(2)}(z) = \left[h_n^{(1)}(z)\right]^* \quad \text{for real } z

    Explicit formulas for low orders:

    .. math::

       h_0^{(2)}(z) &= \frac{e^{-iz}}{-iz} = i \frac{e^{-iz}}{z} \\
       h_1^{(2)}(z) &= -\frac{e^{-iz}}{z}\left(1 - \frac{i}{z}\right) = -\frac{e^{-iz}(z - i)}{z^2}

    Special Values
    --------------
    - h_n^(2)(0) is singular (has a pole at z = 0)
    - h_n^(2)(NaN) = NaN + NaN*i
    ...
    .. math::
       ...
       h_{n+1}^{(2)}(z) = \frac{2n+1}{z} h_n^{(2)}(z) - h_{n-1}^{(2)}(z)
    ...
    .. math::
       ...
       \frac{d}{dz} h_n^{(2)}(z) = \frac{n}{z} h_n^{(2)}(z) - h_{n+1}^{(2)}(z)

    Or equivalently:

    .. math::

       \frac{d}{dz} h_n^{(2)}(z) = h_{n-1}^{(2)}(z) - \frac{n+1}{z} h_n^{(2)}(z)
    ...
    Algorithm
    ---------
    The function is computed using the definition h_n^(2)(z) = j_n(z) - i*y_n(z),
    ...
    Applications
    ------------
    The spherical Hankel function of the second kind appears in many contexts:

    - **Scattering theory**: Represents incoming spherical waves
    - **Electromagnetic radiation**: Multipole fields from external sources
    - **Quantum mechanics**: Asymptotic solutions for scattering problems
    - **Acoustics**: Incoming spherical sound waves
    ...
    -------
    Tensor
        The spherical Hankel function h_n^(2)(z) evaluated at the input values.
    ...
    >>> h2 = sf.spherical_hankel_2(n, z)
    >>> j1 = sf.spherical_bessel_j(n.real, z.real)
    >>> y1 = sf.spherical_bessel_y(n.real, z.real)
    >>> expected = j1 - 1j * y1
    >>> torch.allclose(h2.real, expected.real) and torch.allclose(h2.imag, expected.imag)
    True

    Relation to first kind (conjugate for real z):

    >>> n = torch.tensor([1.0 + 0j])
    >>> z = torch.tensor([2.0 + 0j])
    >>> h1 = sf.spherical_hankel_1(n, z)
    >>> h2 = sf.spherical_hankel_2(n, z)
    >>> torch.allclose(h2, torch.conj(h1))
    True
    ...
    >>> h_nm1 = sf.spherical_hankel_2(n - 1, z)
    >>> h_n = sf.spherical_hankel_2(n, z)
    >>> h_np1 = sf.spherical_hankel_2(n + 1, z)
    >>> lhs = h_nm1 + h_np1
    >>> rhs = (2*n + 1) / z * h_n
    ...
    - For outgoing spherical waves, use spherical_hankel_1 (h_n^(1) = j_n + i*y_n).
    - The spherical Hankel functions are related to the Riccati-Hankel functions by
      chi_n(z) = z * h_n^(2)(z).

    See Also
    --------
    spherical_hankel_1 : Spherical Hankel function of the first kind h_n^(1)
    ...
    return torch.ops.torchscience.spherical_hankel_2(n, z)
